...
int code_ep,code_em,code_el,code_mu; double Zem=0.; double Zep=0.;
        ...
        TVector3 pep,pem, pel,pmu, pmu_in;
...
/*
 ...
 MUonERecoOutputTrack em_out = vrtx.at(j).outgoingMuon();
 MUonERecoOutputTrack ep_out = vrtx.at(j).outgoingElectron();
*/
 MUonERecoOutputTrack mu_in;
 MUonERecoOutputTrack em_out;
 MUonERecoOutputTrack ep_out;
   ...
   if(vrtx.at(j).stationIndex()==1) //and tracks.at(0).processIDofLinkedTrack()==45 and tracks.at(0).linkedTrackID()!=tracks.at(1).linkedTrackID()){
	...
	for(int v=0; v<vrtx.at(j).tracks().size(); v++)
        {
         if(tr_ad.at(v).processIDofLinkedTrack()==5 and vrtx.at(j).tracks().size()>=2 and tr_ad.at(v).sector()==1 and code_em==tr_ad.at(v).linkedTrackID()) 
		{em_out = tr_ad.at(v); cout << em_out.xSlope() <<endl;}
         if(tr_ad.at(v).processIDofLinkedTrack()==5 and vrtx.at(j).tracks().size()>=2 and tr_ad.at(v).sector()==1 and code_ep==tr_ad.at(v).linkedTrackID()) 
		{ep_out =  tr_ad.at(v);}
        }
}
...
if(j==0){
        ...
        gx_v1->SetPoint(gx_v1->GetN(),(6+(1131.7-vrtx.at(j).z())),vrtx.at(j).x());
        gy_v1->SetPoint(gy_v1->GetN(),(6+(1131.7-vrtx.at(j).z())),vrtx.at(j).y());
...
qx_v.push_back(em_out.x0());
mx_v.push_back(em_out.xSlope());
qy_v.push_back(em_out.y0());
my_v.push_back(em_out.ySlope());

qx_v.push_back(ep_out.x0());
mx_v.push_back(ep_out.xSlope());
qy_v.push_back(ep_out.y0());
my_v.push_back(ep_out.ySlope());
		}
       }
}
...
if(tracks.at(j).processIDofLinkedTrack()==5) { TrackIdreco=tracks.at(j).linkedTrackID();}
if(tracks.at(j).processIDofLinkedTrack()==5 and tracks.size()>=2 and tracks.at(j).sector()==1) //and tracks.at(0).processIDofLinkedTrack()==45 and tracks.at(0).linkedTrackID()!=tracks.at(1).linkedTrackID()){
{yes2++;
 ...
 cout << "chi2perDegreeOfFreedom " << tracks.at(j).chi2perDegreeOfFreedom() << endl;
	...
	if(tracks.at(j).linkedTrackID()==code_el)
	{the_rec_v.push_back(sqrt(tracks.at(j).xSlope() * tracks.at(j).xSlope() + tracks.at(j).ySlope() * tracks.at(j).ySlope()));
	 chi_min.push_back(tracks.at(j).chi2perDegreeOfFreedom());}
...
if(tracks.at(j).processIDofLinkedTrack()!=5 or tracks.size()<2) {
