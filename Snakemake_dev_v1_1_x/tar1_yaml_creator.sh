...
outputFile: /mnt/raid10/DATA/espedica/fairmu/TB2025/reco/snakemake/tar1_range_${RANGE}_${NHITS}hit_${INFO}.root
...
inputFiles: [/mnt/raid10/DATA/espedica/fairmu/gen_digi/dev_v1_1_x_${RANGE}_Ideal_tar1.root]

#############################
##### DATA PREPROCESSOR #####
#############################

runDataPreprocessor: false         # Unify different data input (real data/digitization) in a single format ready to be used by re$
...
#   alignmentFile: "alignment_2025_run32.yaml"   # alignment file
  ...
  vertexing:
    ...
    forceVertexReconstructionInTarget: 1
...
eventFilterConfiguration:
  ...
  generatedEvents:
    #minNumberOfMonteCarloTracks: 2         # skip events with < this nb of MC tracks (0 disables cut)

  digitizedEvents:

  preprocessedEvents:
    skimming:
      # filter events based on the offline TriggerResults
      triggerVersion: 3 # (0,1,2,3) (Online was 2 for 2025 runs with RunNumber<24, 3 for runs with RunNumber>=24)
      # only one of these selections applied at a time: "SingleMuonInteraction0","SingleMuonInteraction1","SingleMuonInteraction","PileupMuonInteraction0","PileupMuonInteraction1","PileupMuo$
      skimName: "SingleMuonInteraction1"
