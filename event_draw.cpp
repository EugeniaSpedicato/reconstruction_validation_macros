...
void RealDataAnalyzer(int numb){
	...
	TFile *inputfile = new TFile("TRMesmer_1k.root");//nrrowBP_20GeV_1shared.root");//trPoints,20GeV.root");//exampleProductionJob.root");narrowBP.root
...
{ cout << qx_v.size() << " qx_v"<<endl;
        for(int c=0; c<qx_v.size(); c++)
               {TLine* lx = new TLine(6, trackXatZ(qx_v.at(c),mx_v.at(c), 0.), 12, trackXatZ(qx_v.at(c),mx_v.at(c), 89.9218));
...
{ cout << qy_v.size() << " qy_v"<<endl;
        for(int c=0; c<qy_v.size(); c++)
               {TLine* ly = new TLine(6, trackYatZ(qy_v.at(c),my_v.at(c), 0.), 12, trackYatZ(qy_v.at(c),my_v.at(c), 89.9218));
