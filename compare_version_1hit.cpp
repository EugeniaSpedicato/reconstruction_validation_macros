...
void compare_version_1hit(string path1, string path2, string info1, string info2){


TFile * f_el_pre = TFile::Open(Form("%ssigma_el_pre_1hit_%s.root",path1.c_str(),info1.c_str()));
TFile * f_el_post = TFile::Open(Form("%ssigma_el_post_1hit_%s.root",path1.c_str(),info1.c_str()));
TFile * f_mu_pre = TFile::Open(Form("%ssigma_mu_pre_1hit_%s.root",path1.c_str(),info1.c_str()));
TFile * f_mu_post = TFile::Open(Form("%ssigma_mu_post_1hit_%s.root",path1.c_str(),info1.c_str()));


TFile *f_el_1 = TFile::Open(Form("%sel_1_%s.root",path1.c_str(),info1.c_str()));
TFile *f_mu_1 = TFile::Open(Form("%smu_1_%s.root",path1.c_str(),info1.c_str()));


TFile * f_el_pre_testing =TFile::Open(Form("%ssigma_el_pre_1hit_%s.root",path2.c_str(),info2.c_str()));
TFile * f_el_post_testing = TFile::Open(Form("%ssigma_el_post_1hit_%s.root",path2.c_str(),info2.c_str()));
TFile * f_mu_pre_testing = TFile::Open(Form("%ssigma_mu_pre_1hit_%s.root",path2.c_str(),info2.c_str()));
TFile * f_mu_post_testing = TFile::Open(Form("%ssigma_mu_post_1hit_%s.root",path2.c_str(),info2.c_str()));


TFile *f_el_1_testing = TFile::Open(Form("%sel_1_%s.root",path2.c_str(),info2.c_str()));
TFile *f_mu_1_testing = TFile::Open(Form("%smu_1_%s.root",path2.c_str(),info2.c_str()));
...
TGraph *el_1_testing=(TGraph*)f_el_1_testing->Get("el_1");
TGraph *mu_1_testing=(TGraph*)f_mu_1_testing->Get("mu_1");
...
legend_mu2->AddEntry(mu_1,Form("%s",info1.c_str()),"LEP");
legend_mu2->AddEntry(mu_1_testing,Form("%s",info2.c_str()),"LEP");

 auto legend_e2 = new TLegend(0.75,0.75,0.9,0.9);
legend_e2->AddEntry(el_1,Form("%s",info1.c_str()),"LEP");
legend_e2->AddEntry(el_1_testing,Form("%s",info2.c_str()),"LEP");
...
el_1_testing->SetMarkerColor(kBlue);
mg2->Add(el_1,"A*");
mg2->Add(el_1_testing,"A*");
mg2->Draw("A*");
legend_e2->Draw();
mg2->SetTitle("Sigma difference el 1 hit shared");
...
mu_1_testing->SetMarkerColor(kBlue);
mg4->Add(mu_1,"A*");
mg4->Add(mu_1_testing,"A*");
mg4->Draw("A*");
legend_mu2->Draw();
mg4->SetTitle("Sigma difference #mu 1 hit shared");
mg4->GetYaxis()->SetTitle("(#sigma_post - #sigma_pre)/#sigma_pre");
mg4->GetXaxis()->SetTitle("#theta_#mu (mrad)");

d.SaveAs(Form("%sdiff_1hit_%s.pdf",path2.c_str(),info2.c_str()));
...
mg1pre->Draw("A*");
legend_e1pre->Draw();
mg1pre->SetTitle("Pre sigma el 1 hit shared");
...
mg2post->Draw("A*");
legend_e2post->Draw();
mg2post->SetTitle("Post sigma el 1 hit shared");
...
mg3pre->SetTitle("Pre sigma #mu 1 hit shared");
...
mg4post->Draw("A*");
legend_mu2post->Draw();
mg4post->SetTitle("Post sigma #mu 1 hit shared");
mg4post->GetYaxis()->SetTitle("#sigma_post");
mg4post->GetXaxis()->SetTitle("#theta_#mu (mrad)");
mg4post->SetMinimum(0.0001);

d2.SaveAs(Form("%ssigma_overlap_1hit_%s.pdf",path2.c_str(),info2.c_str()));
...
h_op_LO_clone2->SetTitle("Eff. elastic event VS opening angle 1 hit shared");
...
h_theta_mu_single_LO_clone2->SetTitle("Eff. single particle VS #theta mu 1 hit shared");
...
h_theta_e_single_LO_clone2->SetTitle("Eff. single particle VS #theta e 1 hit shared");
