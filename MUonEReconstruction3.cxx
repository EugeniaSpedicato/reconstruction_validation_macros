		...
		if(m_reconstructionConfiguration.verbose()) std::cout << "stereo hits shared track " << sh_tr.stereoHits().size() << std::endl;
			...
			for(Int_t h=0; h<sh_track_stereo_hits.size(); h++){
			 if(std::find(track_stereo_hits.begin(),track_stereo_hits.end(), sh_track_stereo_hits.at(h))!=track_stereo_hits.end()){removed_hit.push_back(track_stereo_hits.at(h));
										if(m_reconstructionConfiguration.verbose()) std::cout << "module removed hit " << track_stereo_hits.at(h).module().index() << std::endl;}
			}
...
if(count.first > m_reconstructionConfiguration.maxNumberOfSharedHits() || count.second > 0) 
		reconstructedTracksPerSector[sector].erase( std::find_if(it+1, reconstructedTracksPerSector[sector].end(), [&sh_tr](MUonERecoTrack3D& track){
