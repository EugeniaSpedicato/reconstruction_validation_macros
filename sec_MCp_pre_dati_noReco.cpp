...
int sec_MCp_pre_dati_noReco(string version, int nhits,string bend){
...
//   ROOT::TThreadedObject<TH2D> h_2d_pre("h2D_pre","theta mu vs theta E with all cuts", 16, 0.,0.032,NBINS2,edges2);
   ...
   ROOT::TThreadedObject<TH1D> residualX_pre("residualX_pre","residual X_pre",100,-5.,5.);
   ROOT::TThreadedObject<TH1D> residualY_pre("residualY_pre","residual Y_pre",100,-5.,5.);

   ROOT::TThreadedObject<TH1D> residualX_post("residualX_post","residual X_post",1000,-50.,50.);
   ROOT::TThreadedObject<TH1D> residualY_post("residualY_post","residual Y_post",1000,-50.,50.);
...
MUonERecoOutputTrack t_e;
	...
	std::vector<double> pos; pos.resize(6);
      ...
      if(hits_.at(p).moduleID()==0 or hits_.at(p).moduleID()==4) residualX_pre->Fill(hits_.at(p).position()-trackatZ(track.x0(),track.xSlope(),hits_.at(p).z()),wgt);
      if(hits_.at(p).moduleID()==1 or hits_.at(p).moduleID()==5) residualY_pre->Fill(hits_.at(p).position()-trackatZ(track.y0(),track.ySlope(),hits_.at(p).z()),wgt);
	}
...
//	if(hits_e.at(p).moduleID()==1 or hits_e.at(p).moduleID()==5)residualY->Fill(hits_e.at(p).perpendicularResiduum());
      ...
      if(hits_e.at(p).moduleID()==0 or hits_e.at(p).moduleID()==4) residualX_post->Fill(hits_e.at(p).position()-trackatZ(vrtx->outgoingElectron().x0(),vrtx->outgoingElectron().xSlope(),hits_e.at(p).z()),wgt);
      if(hits_e.at(p).moduleID()==1 or hits_e.at(p).moduleID()==5) residualY_post->Fill(hits_e.at(p).position()-trackatZ(vrtx->outgoingElectron().y0(),vrtx->outgoingElectron().ySlope(),hits_e.at(p).z()),wgt);
...
for(int m=0;m<6;m++){
        ...
        double real_pos0= cos(tilt[m])*localX.at(m).at(0);
                            ...
                            if(m==2){ h_hist_distance2->Fill(real_pos1-real_pos0,wgt);
                             h_hist_distance_zoom2->Fill(real_pos1-real_pos0,wgt);
                             }
                } //end for
...
//analog
                             /*h_hist_distance0->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt);
                             h_hist_distance_zoom0->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt);
...
/*  auto h_pos_mu_range3M=h_pos_mu_range3.Merge();
  ...
  auto residualXM_pre = residualX_pre.Merge();
  auto residualYM_pre =residualY_pre.Merge();
  auto residualXM_post = residualX_post.Merge();
  auto residualYM_post =residualY_post.Merge();
...
t.Divide(2,2);
t.cd(1);
residualXM_pre->Draw("hist");
t.cd(2);
residualYM_pre->Draw("hist");
t.cd(3);
residualXM_post->Draw("hist");
t.cd(4);
//residualYM_post->Draw("hist");
h_2d_preMerged->Draw();
t.SaveAs(Form("comparison_RDMC/%s_RD_muin_sig_chi2out50_%s_%dhit_mc.pdf",version.c_str(),bend.c_str(),nhits));



h_2dMerged->SaveAs(Form("comparison_RDMC/%s_2D_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

theta_muMerged->SaveAs(Form("comparison_RDMC/%s_theta_mu_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

theta_eMerged->SaveAs(Form("comparison_RDMC/%s_theta_e_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

d_acoM->SaveAs(Form("comparison_RDMC/%s_d_aco_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

h_openingM->SaveAs(Form("comparison_RDMC/%s_opening_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));


h_2d_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_2D_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

theta_mu_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_theta_mu_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

theta_e_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_theta_e_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

d_aco_preM->SaveAs(Form("comparison_RDMC/%s_preCuts_d_aco_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

h_opening_preM->SaveAs(Form("comparison_RDMC/%s_preCuts_opening_RD_parallel_pre_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

//theqM->SaveAs(Form("comparison_RDMC/%s_th_eqM_pre_Z_mc.pdf",version.c_str(),bend.c_str(),nhits));
...
h_z_posM->Draw("hist");
a.SaveAs(Form("comparison_RDMC/%s_data_pos_sig_chi2out50_%s_%dhit_mc.pdf",version.c_str(),bend.c_str(),nhits));



h_dist_mod2_range0M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_0_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
h_dist_mod2_range1M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_1_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
h_dist_mod2_range2M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_2_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
//h_dist_mod2_range3M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_3_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
//h_dist_mod2_range4M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_4_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
//h_dist_mod2_range5M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_5_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

h_dist_mod0_range0M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_0_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
h_dist_mod0_range1M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_1_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
h_dist_mod0_range2M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_2_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
//h_dist_mod0_range3M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_3_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
//h_dist_mod0_range4M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_4_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
//h_dist_mod0_range5M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_5_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
...
c.SaveAs(Form("comparison_RDMC/%s_h_dist_ranges_el_mu_sig_chi2out50_%s_%dhit_mc.pdf",version.c_str(),bend.c_str(),nhits));
...
p.SaveAs(Form("comparison_RDMC/%s_pos_MOD2_sig_chi2out50_%s_%dhit_mc.pdf",version.c_str(),bend.c_str(),nhits));

TCanvas d("d","d",1400,2100);
d.Divide(2,3);
d.cd(1);
h_dist0M->Draw("hist");
h_dist0M->SaveAs(Form("comparison_RDMC/%s_distance_MOD0_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
d.cd(2);
h_dist1M->Draw("hist");
h_dist1M->SaveAs(Form("comparison_RDMC/%s_distance_MOD1_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
d.cd(3);
h_dist2M->Draw("hist");
h_dist2M->SaveAs(Form("comparison_RDMC/%s_distance_MOD2_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
d.cd(4);
h_dist3M->Draw("hist");
h_dist3M->SaveAs(Form("comparison_RDMC/%s_distance_MOD3_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
d.cd(5);
h_dist4M->Draw("hist");
h_dist4M->SaveAs(Form("comparison_RDMC/%s_distance_MOD4_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));
d.cd(6);
h_dist5M->Draw("hist");
h_dist5M->SaveAs(Form("comparison_RDMC/%s_distance_MOD5_sig_chi2out50_%s_%dhit_mc.root",version.c_str(),bend.c_str(),nhits));

d.SaveAs(Form("comparison_RDMC/%s_distance_sig_chi2out50_%s_%dhit_mc.pdf",version.c_str(),bend.c_str(),nhits));
...
ad.cd(2);
h_dv_acoM->Draw("COLZ");
ad.SaveAs(Form("comparison_RDMC/%s_dist_VS_aco_sig_chi2out50_%s_%dhit_mc.pdf",version.c_str(),bend.c_str(),nhits));
...
n1.SaveAs(Form("comparison_RDMC/%s_distance_stubs_zoom_sig_chi2out50_%s_%dhit_mc.pdf",version.c_str(),bend.c_str(),nhits));
