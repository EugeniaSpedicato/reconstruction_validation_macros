...
int hits = std::stoi(argv[4]);
    ...
    for (auto& entry : fs::directory_iterator(folder1)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".root" and entry.path().string().find("oldVersion") != std::string::npos and entry.path().string().find("possible") == std::string::npos and entry.path().string().find("preVrtx") == std::string::npos and entry.path().string().find("smearing") == std::string::npos) {
