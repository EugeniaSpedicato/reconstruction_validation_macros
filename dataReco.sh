...
outputFile: /mnt/raid10/DATA/espedica/fairmu/TB2025/run${RUN}/${TYPE}/${NAME}_${NHITS}hit.root #used as output filename
...
detectorConfiguration: TR2025_geometry_run11.yaml #no need to give extension (added automatically), configs in run directory are prioritized, if not found, they are looked for in the default directory (common/geometry)
...
inputFiles: [
/eos/experiment/mu-e/staging/daq/2025/decoded/${RUN}/${TYPE}/${NAME}.root
]
...
reconstructionConfiguration:
  ...
  vertexing:
    ...
    useMuonFilterPid: false                                # Use muon filter tag when for PID in vertex
