...
void efficiency_MC(const char* reco_filename, const char* gen_filename, int nhits, const char* index, const char* info,string path){
...
TH1D *h_equal=new TH1D("h_equal","module where hits are shared",6,0,6);
...
// reconstruction efficiency for elastic events including clones of elastic particles
 ...
 std::array<double,6> position_mu{-99.};
 std::array<double,6> position_e{-99.};

 for(int vm=0;vm<hits_mu.size();vm++){if(hits_mu.at(vm).stationID()==1) {position_mu.at(hits_mu.at(vm).moduleID())=hits_mu.at(vm).position();}
					}
 for(int ve=0;ve<hits_e.size();ve++){if(hits_e.at(ve).stationID()==1) {position_e.at(hits_e.at(ve).moduleID())=hits_e.at(ve).position();}
					}

 for(int p=0;p<6;p++){ if(position_e.at(p)==position_mu.at(p) and position_mu.at(p)!=-99. and position_e.at(p)!=-99.){
				h_equal->Fill(p,MesmerEvent->wgt_LO*wnorm);
...
TCanvas ta("ta","ta",700,700);
h_equal->Draw("hist");
ta.SaveAs(Form("%s/hits_shared_%dhit_%s_%s.pdf",path.c_str(),static_cast<char>(nhits),index,info));

TCanvas t("t","t",700,700);
theta_e_single_clone->Draw();
t.SaveAs(Form("%s/el_LO_%dhit_%s_%s.pdf",path.c_str(),static_cast<char>(nhits),index,info));

theta_e_single_clone->SaveAs(Form("%s/theta_e_single_clone_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
theta_mu_single_clone->SaveAs(Form("%s/theta_mu_single_clone_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
theta_e_gen->SaveAs(Form("%s/theta_e_gen_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
theta_mu_gen->SaveAs(Form("%s/theta_mu_gen_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
h_opening_gen->SaveAs(Form("%s/h_opening_gen_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
theta_e_clone->SaveAs(Form("%s/theta_e_clone_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
theta_mu_clone->SaveAs(Form("%s/theta_mu_clone_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
h_opening_clone->SaveAs(Form("%s/h_opening_clone_%dhit_%s_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
...
gStyle->SetOptStat(0);
a1.SaveAs(Form("%s/eff_LO_%dhit_%s.pdf",path.c_str(),static_cast<char>(nhits),index,info));


h0_clone->SaveAs(Form("%s/el_single_eff_LO_%dhit_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
h1_clone->SaveAs(Form("%s/mu_single_eff_LO_%dhit_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
h2_clone->SaveAs(Form("%s/el_eff_LO_%dhit_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
h3_clone->SaveAs(Form("%s/mu_eff_LO_%dhit_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
h4_clone->SaveAs(Form("%s/op_eff_LO_%dhit_%s.root",path.c_str(),static_cast<char>(nhits),index,info));
*/
}
