...
if(stubs_muin==6 and abs(posxIN)<=1.5 and abs(posyIN)<=1.5 and chi2_muin<2){
 ...
 if(abs(acoplanarity_v)<=1 and chi<20 and vrtx->muonTheta()>0.0002){
	...
	if(vrtx->electronTheta()>0.0 and vrtx->electronTheta()<=0.005){h_res->Fill(vrtx->electronTheta()-the_gen,mesmerEvent->wgt_full);
									if(vrtx->electronTheta()-the_gen>0.001){thetaX->Fill(e_out.xSlope(),mesmerEvent->wgt_full);
									thetaY->Fill(e_out.ySlope(),mesmerEvent->wgt_full);
                                                                        thetaXg->Fill(theX_gen,mesmerEvent->wgt_full);
                                                                        thetaYg->Fill(theY_gen,mesmerEvent->wgt_full);}
	}
	else{                                                           if(vrtx->electronTheta()-the_gen<-0.01 ){thetaX2->Fill(e_out.xSlope(),mesmerEvent->wgt_full);
                                                                        thetaY2->Fill(e_out.ySlope(),mesmerEvent->wgt_full);
...
b.SaveAs("projections_nobend.pdf");
