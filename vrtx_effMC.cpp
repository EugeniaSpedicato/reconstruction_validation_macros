...
TH1D *h_opening_clone_reco=new TH1D("h_opening_clone_reco", "Reco angle reco events from MESMER with clones",33,0.,0.033);

TH1D *h_opening_clone_vrtx_reco=new TH1D("h_opening_clone_vrtx_reco", "Reco opening angle reco events from MESMER with clones when vrtx is created with good quality",33,0.,0.033);

TH1D *h_opening_wrong_vrtx_reco=new TH1D("h_opening_wrong_vrtx_reco", "Reco opening angle reco events from MESMER with clones when vrtx is created with wrong association",33,0.,0.033);
...
if(stubs_muin>=5 and chi2_muin<2){//and thmu_gen>0.0002){//and E_e>1){
 ...
 if(e>=1 and mu>=1 and find_if(quality_e.begin(),quality_e.end(),[](double i){return i>=0.65;})!=end(quality_e) and find_if(quality_mu.begin(),quality_mu.end(),[](double i){return i>=0.65;})!=end(quality_mu)){

 double opening_angle_reco=p_mu.Angle(p_e);

 h_opening_clone->Fill(opening_angle,MesmerEvent->wgt_LO*wnorm);
 h_opening_clone_reco->Fill(opening_angle,MesmerEvent->wgt_LO*wnorm);

  if(chi!=0 and ((vrtx.outgoingMuon().linkedTrackID()==code_e and vrtx.outgoingElectron().linkedTrackID()==code_mu) or (vrtx.outgoingMuon().linkedTrackID()==code_mu and vrtx.outgoingElectron().linkedTrackID()==code_e)) ){
	 h_opening_clone_vrtx->Fill(opening_angle,MesmerEvent->wgt_LO*wnorm);
         h_opening_clone_vrtx_reco->Fill(opening_angle_reco,MesmerEvent->wgt_LO*wnorm);
	}//chi!=0
  else{   n_wrong+=MesmerEvent->wgt_LO*wnorm;
          h_opening_wrong_vrtx->Fill(opening_angle,MesmerEvent->wgt_LO*wnorm);
          h_opening_wrong_vrtx_reco->Fill(opening_angle_reco,MesmerEvent->wgt_LO*wnorm);}
...
TH1D * h3vrtx = (TH1D*) h_opening_clone_vrtx_reco->Clone();
TH1D * h3 = (TH1D*) h_opening_clone_reco->Clone();
h3vrtx->Divide(h3vrtx,h3,1,1,"B");
...
h3vrtx_w->Divide(h3vrtx_w,h3_w,1,1,"B");
...
a.cd(4);
h3vrtx_w->Draw();
a.SaveAs("proposal/vrtx_LO_0hit_quality.pdf");

h2vrtx->SaveAs("proposal/VRTX_correct_LO_0hit_true_clone_quality.root");
h3vrtx->SaveAs("proposal/VRTX_correct_LO_0hit_reco_clone_quality.root");

h2vrtx_w->SaveAs("proposal/VRTX_wrong_LO_0hit_true_clone_quality.root");
h3vrtx_w->SaveAs("proposal/VRTX_wrong_LO_0hit_reco_clone_quality.root");
