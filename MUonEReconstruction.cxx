...
if(m_reconstructionConfiguration.verbose())
std::cout << "N tracks after removal " <<  reconstructedTracksPerSector[sector].size() << " tracks"<< std::endl;
...
std::cout << "how many stereo!? " << nstereo << std::endl;
		...
		if(m_reconstructionConfiguration.verbose()) std::cout << "for loop " << stubmodule_index << std::endl;
					...
					if(!m_fitter.addFitInfo(tr, 0 == sector ? m_reconstructionConfiguration.allowTrivialIncomingTracks() : false, m_reconstructionConfiguration.maxOutlierChi2()))
					{
                                ++it;
