...
int main(int argc, char* argv[]) {
    ...
    for (auto& entry : fs::directory_iterator(folder)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".txt" and entry.path().string().find("4_files") == std::string::npos) {
            files.push_back(entry.path());
        }
    }

std::cout << "files_1.size() " << files.size() << std::endl;

    fs::path folder2 = argv[2];
    if (!fs::exists(folder2) || !fs::is_directory(folder2)) {
        std::cerr << "Cartella non valida: " << folder2 << std::endl;
        return 1;
    }
    for (auto& entry : fs::directory_iterator(folder2)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".txt" and entry.path().string().find("possible") == std::string::npos and entry.path().string().find("oldVersion") == std::string::npos and entry.path().string().find("4_files") == std::string::npos and entry.path().string().find("smearing") == std::string::npos and entry.path().string().find("preVrtx") == std::string::npos and entry.path().string().find("zin3") == std::string::npos) {
            files.push_back(entry.path());
        }
    }

std::cout << "files_1+2.size() " << files.size() << std::endl;
        ...
        if (filename.find("single_muon_interaction_0") != std::string::npos) {
            while (std::getline(infile, line)) {
                if (std::regex_search(line, match, pattern_num_fid_tr0) && match.size() > 1) {
                    //std::cerr << filename << " -> Valore num_fid tr0: " << match[1] << std::endl;
                    ...
                    //std::cerr << filename << " -> Valore num_el tr0: " << match[1] << std::endl;
                    ...
                    //std::cerr << filename << " -> Valore num_bkg tr0: " << match[1] << std::endl;
                    ...
                    //std::cerr << filename << " -> Valore num_sig tr0: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore fid tr0: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore vrtx tr0: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore preEl tr0: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore el tr0: " << match[1] << std::endl;
                    x0_el.push_back(num);
            ...
            while (std::getline(infile, line)) {
                if (std::regex_search(line, match, pattern_num_fid_tr1) && match.size() > 1) {
                    //std::cerr << filename << " -> Valore num_fid tr1: " << match[1] << std::endl;
                    ...
                    //std::cerr << filename << " -> Valore num_el tr1: " << match[1] << std::endl;
                    ...
                    //std::cerr << filename << " -> Valore num_bkg tr1: " << match[1] << std::endl;
                    ...
                    //std::cerr << filename << " -> Valore num_sig tr1: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore fid t10: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore vrtx tr1: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore preEl tr1: " << match[1] << std::endl;
...
//                    std::cerr << filename << " -> Valore el tr1: " << match[1] << std::endl;
                    x1_el.push_back(num);
...
std::cerr<<"Elastic vrtx0 " << tot_el_0 << " +- " << sqrt(tot_el_0) << std::endl;
std::cerr<<"Elastic vrtx1 " << tot_el_1 << " +- " << sqrt(tot_el_1) << std::endl;

std::cerr<<"FIducial vrtx0 " << tot_fid_0 << " +- " << sqrt(tot_fid_0) << std::endl;
std::cerr<<"FIducial vrtx1 " << tot_fid_1 << " +- " << sqrt(tot_fid_1) << std::endl;


std::cerr<<"Total ratio el/fid vrtx0 " << tot_el_0/tot_fid_0 << " +- " <<
...
<< std::endl;
std::cerr<<"Total ratio el/fid vrtx1 " << tot_el_1/tot_fid_1 << " +- " <<
...
std::cerr<<"Total ratio bkg/fid vrtx0 " << tot_bkg_0/tot_fid_0 << " +- " <<
...
<< std::endl;
std::cerr<<"Total ratio bkg/fid vrtx1 " << tot_bkg_1/tot_fid_1 << " +- " <<
...
std::cerr<<"Total ratio sig/fid vrtx0 " << tot_sig_0/tot_fid_0 << " +- " <<
...
<< std::endl;
std::cerr<<"Total ratio sig/fid vrtx1 " << tot_sig_1/tot_fid_1 << " +- " <<
...
gPad->BuildLegend(0.55,0.6,0.85,0.8);
        ...
        c->SaveAs(Form("wip11x_ratevstime_fiducial_run%s.pdf",argv[3]));
