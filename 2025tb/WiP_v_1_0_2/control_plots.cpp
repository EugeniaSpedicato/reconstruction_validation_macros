...
//TFile* f_0 = TFile::Open(Form("txt/run%i/root_single_muon_interaction_0_%s.root",run,file.c_str(),hits));
//TFile* f_1 = TFile::Open(Form("txt/run%i/root_single_muon_interaction_1_%s.root",run,file.c_str(),hits));


if (argc < 5) {
    std::cerr << "Usage: " << argv[0] << " folder0 folder1 run hits\n";
    return 1;
}
for(int i=0;i<argc;i++)
    std::cout << i << " : " << argv[i] << std::endl;

    fs::path folder0 = argv[1];
    fs::path folder1 = argv[2];
    std::vector<fs::path> files_0;
    std::vector<fs::path> files_1;

int run = std::stoi(argv[3]);
int hits = std::stoi(argv[4]);


    for (auto& entry : fs::directory_iterator(folder0)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".root" and entry.path().string().find("4_files") == std::string::npos) {
            if(entry.path().string().find("single_muon_interaction_0") != std::string::npos){files_0.push_back(entry.path());}
        }
    }
    for (auto& entry : fs::directory_iterator(folder1)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".root" and entry.path().string().find("possible") == std::string::npos and entry.path().string().find("oldVersion") == std::string::npos and entry.path().string().find("4_files") == std::string::npos and entry.path().string().find("preVrtx") == std::string::npos and entry.path().string().find("smearing") == std::string::npos and entry.path().string().find("zin3") == std::string::npos) {
	    if(entry.path().string().find("single_muon_interaction_1") != std::string::npos){files_1.push_back(entry.path());}
        }
    }
...
//std::vector<TH2D*> h_final_rndm_0(files_0.size(), nullptr);
...
//std::vector<TH2D*> h_final_rndm_1(files_1.size(), nullptr);
...
//h_final_rndm_0.at(f)=(TH2D*)f_0->Get("h_final_rndm_0");
...
//h_final_rndm_0.at(0)->Add(h_final_rndm_0.at(f));
	}

}
std::cout << "files_1.size() " << files_1.size() << std::endl;
std::cout << "files_0.size() " << files_0.size() << std::endl;
...
//h_final_rndm_1.at(f)=(TH2D*)f_0->Get("h_final_rndm_1");
...
//h_final_rndm_1.at(0)->Add(h_final_rndm_1.at(f));
		}
}
...
//h_final_rndm_0.at(0)->Scale(1./h_final_rndm_0.at(0)->Integral());
...
//h_final_rndm_1.at(0)->Scale(1./h_final_rndm_1.at(0)->Integral());
...
pe.SaveAs(Form("plots/run%i/preEl_nhits%i.root",run,hits));
...
fe.SaveAs(Form("plots/run%i/finalEl_nhits%i.pdf",run,hits));
h_E_th_min_0.at(0)->SaveAs(Form("plots/run%i/thetas/thetaMin0_nhits%i.root",run,hits));
h_E_th_min_1.at(0)->SaveAs(Form("plots/run%i/thetas/thetaMin1_nhits%i.root",run,hits));
h_E_th_max_0.at(0)->SaveAs(Form("plots/run%i/thetas/thetaMax0_nhits%i.root",run,hits));
h_E_th_max_1.at(0)->SaveAs(Form("plots/run%i/thetas/thetaMax1_nhits%i.root",run,hits));
...
gen_fin.SaveAs(Form("plots/run%i/final_nhits%i.pdf",run,hits));
gen_fin.SaveAs(Form("plots/run%i/final_nhits%i.root",run,hits));

/*TCanvas umb("umb","umb",1000,500);
umb.Divide(2,1);
umb.cd(1);
h_final_rndm_0.at(0)->Draw();
umb.cd(2);
h_final_rndm_1.at(0)->Draw();
umb.SaveAs(Form("plots/run%i/final_umb_nhits%i.root",run,hits));
*/
