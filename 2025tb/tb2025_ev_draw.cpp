...
//double z_mod[6]={911.2+18.0218,911.2+21.8693,911.2+55.3635,911.2+56.6205,911.2+89.9218,911.2+92.3722};
...
//        if(nhits==0 and version==2025 and interaction==0){name="/mnt/raid10/DATA/espedica/fairmu/TB2025/run19/single_muon_interaction_0/muedaq03-1752991125_0hit.root";}
//        else if(nhits==0 and version==2025 and interaction==1){name="/mnt/raid10/DATA/espedica/fairmu/TB2025/run19/single_muon_interaction_1/muedaq03-1752991125_0hit.root";}
...
if(chi==0){
	...
	for(int h=0;h<hits_.size();h++){position_in.at(n0).at(hits_.at(h).moduleID()).push_back(hits_.at(h).positionPerpendicular());}
	...
	for(int h=0;h<hits_.size();h++){position.at(n).at(hits_.at(h).moduleID()).push_back(hits_.at(h).positionPerpendicular());
	...
	for(int h=0;h<hits_.size();h++){position_other.at(n1).at(hits_.at(h).moduleID()).push_back(hits_.at(h).positionPerpendicular());}
	n1++;
	}
}
