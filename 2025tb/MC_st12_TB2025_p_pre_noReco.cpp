...
int MC_TB2025_p_pre_noReco(string version, int nhits,string bend,string mc, string smear){
...
if(version=="MC" and nhits==0){
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/MC/50k_tar1_pt2.root");
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/MC/1M_tar0.root");}
...
TH1::SetDefaultSumw2(kTRUE);
   ...
   ROOT::TThreadedObject<TH1D> theta_e_post("theta_e_post", "Electron scattering reco angles post vrtx",64,0.,0.032);//32,0.,0.032);
   ROOT::TThreadedObject<TH1D> theta_mu_post("theta_mu_post", "Muon scattering reco angles post vrtx",100,0.,0.005);//50,0.,0.005);
   ROOT::TThreadedObject<TH1D> theta_e("theta_e", "Electron scattering reco angles",64,0.,0.032);//32,0.,0.032);
   ROOT::TThreadedObject<TH1D> theta_mu("theta_mu", "Muon scattering reco angles",100,0.,0.005);//50,0.,0.005);
   ROOT::TThreadedObject<TH1D> h_opening("h_opening", "Opening angle reco events",64,0.,0.032);//32,0.,0.032);
   ROOT::TThreadedObject<TH1D> d_aco("d_aco_real", "Acoplanarity",600,-3.2,3.2);



   ROOT::TThreadedObject<TH1D> theta_e_pre("theta_e_pre", "Electron scattering reco angles pre-cuts",64,0.,0.032);//32,0.,0.032);
   ROOT::TThreadedObject<TH1D> theta_mu_pre("theta_mu_pre", "Muon scattering reco angles pre-cuts",100,0.,0.005);//50,0.,0.005);
   ROOT::TThreadedObject<TH1D> h_opening_pre("h_opening_pre", "Opening angle reco events pre-cuts",64,0.,0.032);//32,0.,0.032);
   ROOT::TThreadedObject<TH1D> d_aco_pre("d_aco_real_pre", "Acoplanarity pre-cuts",600,-3.2,3.2);

   ROOT::TThreadedObject<TH1D> theq("theq", "Equal angles",100,0.,0.005);//50,0.,0.005);
...
//   ROOT::TThreadedObject<TH2D> h_2d_pre("h2D_pre","theta mu vs theta E with all cuts", 16, 0.,0.032,NBINS2,edges2);
   ...
   ROOT::TThreadedObject<TH2D> h_2d("h2D","theta mu vs theta E with all cuts", 320, 0.,0.032,100,0.,0.005);//50,0.,0.005);
   ROOT::TThreadedObject<TH2D> h_2d_pre("h2D_pre","theta mu vs theta E with all cuts", 320, 0.,0.032,100,0.,0.005);//50,0.,0.005);
...
*/
   ...
   ROOT::TThreadedObject<TH1D> count_all("count_all","count_all",2,0.,2.);
   ROOT::TThreadedObject<TH1D> count_all_w2("count_all_w2","count_all_w2",2,0.,2.);
   ROOT::TThreadedObject<TH1D> count_w2("count_w2","count_w2",2,0.,2.);
   ROOT::TThreadedObject<TH1D> count_w2_sel("count_w2_sel","count_w2_sel",2,0.,2.);
   ROOT::TThreadedObject<TH1D> count_w_sel("count_w_sel","count_w_sel",2,0.,2.);
   ...
   ROOT::TThreadedObject<TH1D> theta_L("theta_L", "Left scattering reco angles",96,0.,0.032);//32,0.,0.032);
   ROOT::TThreadedObject<TH1D> theta_R("theta_R", "Right scattering reco angles",96,0.,0.032);//50,0.,0.005);
   ROOT::TThreadedObject<TH2D> theta_RL("theta_RL", "right VS left scattering angles",96,0.,0.032,96,0.,0.032);
...
if(version=="my_modifica_norun")wf="wgt_norun";
else if(version=="my_modifica_LO" or version=="my_modifica_LO_pure")wf="wgt_LO";
else if(version=="my_modifica_NLO")wf="wgt_NLO";
...
count_all->Fill(1.,wgt*factor);
count_all_w2->Fill(1.,wgt*wgt*factor*factor);

double the_rec=-99.;
double thmu_rec=-99.;
TVector3 p_e,p_mu,p_muin,p_muin_vrtx;
        ...
        if(track.sector()==1) sec0++;
        if(track.sector()==2) sec1++;
        }
...
MUonERecoOutputTrackAnalysis t_e;
        ...
        if(track.sector()==1 and sec0==1){
	stubs_muin=hits_.size();
	if(smear=="yes"){
        th_inx=track.xSlope()+gRandom->Gaus(0.,1.7e-05);//1.7e-05
        th_iny=track.ySlope()+gRandom->Gaus(0.,1.7e-05);
	}
	else{
        th_inx=track.xSlope();
        th_iny=track.ySlope();
	...
	h_chi2->Fill(chi2_muin,wgt*factor);
	h_xslope->Fill(th_inx,wgt*factor);
                        }
	if(track.sector()==2)
	{
	...
	if(smear=="yes"){
	if(std::equal(pos.begin(),pos.end(),pos_mu.begin())){p_mu.SetXYZ(track.xSlope()+gRandom->Gaus(0.,1.7e-05),track.ySlope()+gRandom->Gaus(0.,1.7e-05),1.0); p_mu.Unit(); thmu_rec=p_mu.Angle(p_muin);t_mu=track;}
       else if(std::equal(pos.begin(),pos.end(),pos_e.begin())){p_e.SetXYZ(track.xSlope()+gRandom->Gaus(0.,1.7e-05),track.ySlope()+gRandom->Gaus(0.,1.7e-05),1.0); p_e.Unit(); the_rec=p_e.Angle(p_muin);t_e=track;}
	}
	else{
...
//post vertex
/*      if(smear=="yes"){
        p_muin_vrtx.SetXYZ(vrtx->incomingMuon().xSlope()+gRandom->Gaus(0.,1.7e-05),vrtx->incomingMuon().ySlope()+gRandom->Gaus(0.,1.7e-05),1.0);
        p_muin_vrtx=p_muin_vrtx.Unit();
         if(std::equal(pos.begin(),pos.end(),pos_mu.begin())){p_mu.SetXYZ(vrtx->outgoingMuon().xSlope()+gRandom->Gaus(0.,1.7e-05),vrtx->outgoingMuon().ySlope()+gRandom->Gaus(0.,1.7e-05),1.0); p_mu.Unit(); thmu_rec=p_mu.Angle(p_muin_vrtx);t_mu=track;}//thmu_rec=vrtx->muonTheta();t_mu=track;}
      else if(std::equal(pos.begin(),pos.end(),pos_e.begin())){p_e.SetXYZ(vrtx->outgoingElectron().xSlope()+gRandom->Gaus(0.,1.7e-05),vrtx->outgoingElectron().ySlope()+gRandom->Gaus(0.,1.7e-05),1.0); p_e.Unit(); the_rec=p_e.Angle(p_muin_vrtx);t_e=track;}//the_rec=vrtx->electronTheta();t_e=track;}
}
else{
        p_muin_vrtx.SetXYZ(vrtx->incomingMuon().xSlope(),vrtx->incomingMuon().ySlope(),1.0);
        p_muin_vrtx=p_muin_vrtx.Unit();
         if(std::equal(pos.begin(),pos.end(),pos_mu.begin())){p_mu.SetXYZ(vrtx->outgoingMuon().xSlope(),vrtx->outgoingMuon().ySlope(),1.0); p_mu.Unit(); thmu_rec=p_mu.Angle(p_muin_vrtx);t_mu=track;}//thmu_rec=vrtx->muonTheta();t_mu=track;}
      else if(std::equal(pos.begin(),pos.end(),pos_e.begin())){p_e.SetXYZ(vrtx->outgoingElectron().xSlope(),vrtx->outgoingElectron().ySlope(),1.0); p_e.Unit(); the_rec=p_e.Angle(p_muin_vrtx);t_e=track;}//the_rec=vrtx->electronTheta();t_e=track;}
}
*/
...
int stub1 = 0;
        ...
        std::array<std::vector<float>,6> localX{{std::vector<float>(0.0)}};
         for (auto&& stub : stubs) {
                if(stub.stationID()==1){stub0++; if(stub.moduleID()==4){posxIN=stub.position(); } else if(stub.moduleID()==5){posyIN=stub.position();}   }
                if(stub.stationID()==2){stub1++; module_st1_2.at(stub.moduleID())+=1; module_st1.at(stub.moduleID())=1;int link= stub.moduleID(); localX.at(link).push_back(stub.position());}
                //if(stub.stationID()==1){stub1++; module_st1_2.at(stub.moduleID())+=1;int link= stub.moduleID(); localX.at(link).push_back(stub.position());}
        }

if(chi!=0){
        h_e_pre->Fill(the_rec,wgt*factor);
        h_e_post->Fill(vrtx->electronTheta(),wgt*factor);
        h_mu_pre->Fill(thmu_rec,wgt*factor);
        h_mu_post->Fill(vrtx->muonTheta(),wgt*factor);
        h_inc_pre->Fill(th_muin,wgt*factor);
        h_inc_post->Fill(p_muin_v.Theta(),wgt*factor);
...
// if(abs(posxIN)<=1.5 and abs(posyIN)<=1.5 and stub0==6){// and stub1<=15){
if( abs(posxIN)<=1.4985 and abs(posyIN)<=1.4985 and stub0==6 and sec0==1 and th_muin<0.004){
...
count->Fill(1.,wgt*factor);
count_w2->Fill(1.,wgt*wgt*factor*factor);
...
for(int m=0;m<6;m++){
        ...
        double real_pos0= cos(tilt[m])*localX.at(m).at(0);
                            if(m==0){ h_hist_distance0->Fill(real_pos1-real_pos0,wgt*factor);
                             h_hist_distance_zoom0->Fill(real_pos1-real_pos0,wgt*factor);
                             }

                            if(m==2){ h_hist_distance2->Fill(real_pos1-real_pos0,wgt*factor);
                             h_hist_distance_zoom2->Fill(real_pos1-real_pos0,wgt*factor); u=real_pos1-real_pos0;
                             }
...
//        h_z_pos_pre->Fill(vrtx->zPositionFit()+11.200300);
        h_z_pos_pre->Fill(vrtx->zPositionFit());

//double zpos = vrtx->zPositionFit()+11.200300;
...
TVector3 p_muin_post(vrtx->incomingMuon().xSlope(),vrtx->incomingMuon().ySlope(),1.0); p_muin_post.Unit();
                                                ...
                                                double dotProduct_v_post = p_mu_post.Dot(p_e_post);
                                                TVector3 crossProduct_v_post = p_mu_post.Cross(p_e_post);
                                                double T_v_post = p_muin_post.Dot(crossProduct_v_post);
                                                TVector3 im_v_post= p_muin_post.Cross(p_mu_post);
                                                TVector3 ie_v_post= p_muin_post.Cross(p_e_post);
                                                T_v_post = T_v_post>0? 1:-1;
                                                double acoplanarity_v_post= T_v_post*(TMath::Pi()- acos( ((im_v_post).Dot(ie_v_post))/(im_v_post.Mag()*ie_v_post.Mag()) ));



 d_aco_pre->Fill(acoplanarity_v,wgt*factor);
 h_2d_pre->Fill(the_rec,thmu_rec,wgt*factor);
 theta_mu_pre->Fill(thmu_rec,wgt*factor);
 theta_e_pre->Fill(the_rec,wgt*factor);
 h_opening_pre->Fill(p_mu.Angle(p_e),wgt*factor);
 h_nstubs_pre_s1->Fill(stub1,wgt*factor);
...
// if(chi>0 and stub1<=15 and thmu_rec>0.0002 and the_rec<0.032 and the_rec>0.000 and thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002 and abs(acoplanarity_v)<=0.4 and zpos>906 ){
//questo
// if(chi>0 and stub1<=15 and thmu_rec>0.0002 and the_rec<0.02 and the_rec>=0.005 and thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002 and p_mu.Angle(p_e)>0.005 and abs(acoplanarity_v)<=0.4 and zpos>906 ){
 //if(chi>0 and stub1<=15 and thmu_rec>0.0002 and the_rec<0.02 and the_rec>=0.005 and thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002 and p_mu.Angle(p_e)>0.005 and abs(acoplanarity_v)<=0.4 and zpos>896 ){
if( abs(posxIN)<=1.4985 and abs(posyIN)<=1.4985 and stub0<10 and sec0==1 and th_muin<0.004){
...
//double ms=0.0136/p * sqrt(1.5/19.32) * (1. + 0.038 * log(1.5/19.32));


//double ms=sqrt( pow(0.0136/p * sqrt(1.5/19.32) * (1. + 0.038 * log(1.5/19.32)),2) + pow(0.0136/p * sqrt(0.032*12/9.37) * (1. + 0.038 * log(0.032*12/9.37)),2));
...
if(t_e.xSlope()>t_mu.xSlope()){theta_R->Fill(the_rec,wgt*factor); theta_L->Fill(thmu_rec,wgt*factor); theta_RL->Fill(the_rec,thmu_rec,wgt*factor); }
else{theta_R->Fill(thmu_rec,wgt*factor); theta_L->Fill(the_rec,wgt*factor); theta_RL->Fill(thmu_rec,the_rec,wgt*factor);}

vrtx_chi2->Fill(chi,wgt*factor);
track_el->Fill(t_e.chi2perDegreeOfFreedom(),wgt*factor);
track_mu->Fill(t_mu.chi2perDegreeOfFreedom(),wgt*factor);
        ...
        h_nstubs_track_e->Fill(t_e.hits().size(),wgt*factor);
        h_nstubs_track_mu->Fill(t_mu.hits().size(),wgt*factor);
...
//analog
 ...
 if( t_e_hits.at(0)!=-99. and t_mu_hits.at(0)!=-99.) h_dist0->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt*factor);
 if( t_e_hits.at(1)!=-99. and t_mu_hits.at(1)!=-99.) h_dist1->Fill(t_e_hits.at(1)-t_mu_hits.at(1),wgt*factor);
 if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.) h_dist2->Fill(t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor);
 if( t_e_hits.at(3)!=-99. and t_mu_hits.at(3)!=-99.) h_dist3->Fill(t_e_hits.at(3)-t_mu_hits.at(3),wgt*factor);
 if( t_e_hits.at(4)!=-99. and t_mu_hits.at(4)!=-99.) h_dist4->Fill(t_e_hits.at(4)-t_mu_hits.at(4),wgt*factor);
 if( t_e_hits.at(5)!=-99. and t_mu_hits.at(5)!=-99.) h_dist5->Fill(t_e_hits.at(5)-t_mu_hits.at(5),wgt*factor);


     if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.)h_du_aco->Fill(acoplanarity_v,t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor);
     if( t_e_hits.at(3)!=-99. and t_mu_hits.at(3)!=-99.)h_dv_aco->Fill(acoplanarity_v,t_e_hits.at(3)-t_mu_hits.at(3),wgt*factor);

/*
if( the_rec<=0.005 ){ if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){h_dist_mod2_range0->Fill(t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor); h_pos_range0->Fill(t_e_hits.at(2),t_mu_hits.at(2),wgt*factor);
                                                                         h_pos_el_range0->Fill(t_e_hits.at(2),wgt*factor); h_pos_mu_range0->Fill(t_mu_hits.at(2),wgt*factor);}
                      if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){h_dist_mod0_range0->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt*factor);}}

if( the_rec>0.005 and the_rec<=0.0075 ){ if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){ h_dist_mod2_range1->Fill(t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor);h_pos_range1->Fill(t_e_hits.at(2),t_mu_hits.at(2),wgt*factor);
                                                                         h_pos_el_range1->Fill(t_e_hits.at(2),wgt*factor); h_pos_mu_range1->Fill(t_mu_hits.at(2),wgt*factor);}
                      if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){h_dist_mod0_range1->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt*factor);}}

if( the_rec>0.0075 and the_rec<=0.01 ){ if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){ h_dist_mod2_range2->Fill(t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor);h_pos_range2->Fill(t_e_hits.at(2),t_mu_hits.at(2),wgt*factor);
                                                                         h_pos_el_range2->Fill(t_e_hits.at(2),wgt*factor); h_pos_mu_range2->Fill(t_mu_hits.at(2),wgt*factor);}
                      if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){h_dist_mod0_range2->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt*factor);}}

if( the_rec>0.010 and the_rec<=0.015 ){ if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){ h_dist_mod2_range3->Fill(t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor);h_pos_range3->Fill(t_e_hits.at(2),t_mu_hits.at(2),wgt*factor);
                                                                         h_pos_el_range3->Fill(t_e_hits.at(2),wgt*factor); h_pos_mu_range3->Fill(t_mu_hits.at(2),wgt*factor);}
                      if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){h_dist_mod0_range3->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt*factor);}}

if( the_rec>0.015 and the_rec<=0.025 ){ if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){ h_dist_mod2_range4->Fill(t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor);h_pos_range4->Fill(t_e_hits.at(2),t_mu_hits.at(2),wgt*factor);
                                                                         h_pos_el_range4->Fill(t_e_hits.at(2),wgt*factor); h_pos_mu_range4->Fill(t_mu_hits.at(2),wgt*factor);}
                      if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){h_dist_mod0_range4->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt*factor);}}

if( the_rec>0.025 and the_rec<=0.035 ){ if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){ h_dist_mod2_range5->Fill(t_e_hits.at(2)-t_mu_hits.at(2),wgt*factor);h_pos_range5->Fill(t_e_hits.at(2),t_mu_hits.at(2),wgt*factor);
                                                                         h_pos_el_range5->Fill(t_e_hits.at(2),wgt*factor); h_pos_mu_range5->Fill(t_mu_hits.at(2),wgt*factor);}
                      if( t_e_hits.at(2)!=-99. and t_mu_hits.at(2)!=-99.){h_dist_mod0_range5->Fill(t_e_hits.at(0)-t_mu_hits.at(0),wgt*factor);}}
*/


 h_z_pos->Fill(vrtx->zPositionFit()+11.200300);
 d_aco->Fill(acoplanarity_v,wgt*factor);
 h_2d->Fill(the_rec,thmu_rec,wgt*factor);
 theta_mu->Fill(thmu_rec,wgt*factor);
 theta_e->Fill(the_rec,wgt*factor);
 h_opening->Fill(p_mu.Angle(p_e),wgt*factor);
 h_nstubs_s1->Fill(stub1,wgt*factor);


 theta_mu_post->Fill(vrtx->muonTheta());
 theta_e_post->Fill(vrtx->electronTheta());

 count_w_sel->Fill(1.,wgt*factor);
 count_w2_sel->Fill(1.,wgt*wgt*factor*factor);

if(thmu_rec>=the_rec-0.00005 and thmu_rec<=the_rec+0.00005)theq->Fill(thmu_rec,wgt*factor);
				}//yes2>=2
			}//aco e chi
		}//chi!=0
	}//mu_in
...
auto count_allM=count_all.Merge();
auto count_all_w2M=count_all_w2.Merge();
auto countM = count.Merge();
auto count_w2M = count_w2.Merge();
auto count_w2_selM = count_w2_sel.Merge();
auto count_w_selM = count_w_sel.Merge();
...
/*  auto h_pos_mu_range3M=h_pos_mu_range3.Merge();
  ...
  auto theta_mu_postM=theta_mu_post.Merge();
  auto theta_e_postM=theta_e_post.Merge();

  auto theta_RM=theta_R.Merge();
  auto theta_LM=theta_L.Merge();
  auto theta_RLM=theta_RL.Merge();
...
t.SaveAs(Form("comparison_RDMC/%s_RD_muin_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
*/

cout << "count_allM number of entries " << count_allM->GetEntries() << endl;
cout << "all sample weights count_allM.Integral() " << count_allM->Integral() << endl;
cout << "w2 of all sample count_all_w2M.Integral() " << count_all_w2M->Integral() << endl;
cout << "fiducial sample number of entries " << countM->GetEntries() << endl;
cout << "fiducial sample weights countM.Integral() " << countM->Integral() << endl;
cout << "w2 of fiducial sample count_w2M.Integral() " << count_w2M->Integral() << endl;
cout << "elastic sample number of entries " << count_w_selM->GetEntries() << endl;
cout << "elastic sample weights count_w_selM.Integral() " << count_w_selM->Integral() << endl;
cout << "w2 of elastic events count_w2_selM.Integral() " << count_w2_selM->Integral() << endl;


string sm=".";
if(smear=="yes")sm="_smearing.";

vrtx_chi2M->SaveAs(Form("comparison_RDMC/%s_vrtx_chi2_basicSelection_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
track_elM->SaveAs(Form("comparison_RDMC/%s_track_el_basicSelection_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
track_muM->SaveAs(Form("comparison_RDMC/%s_track_mu_basicSelection_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
...
if (h_2d_preMerged->GetBinContent(i,j)<100) h_2d_preMerged->SetBinContent(i,j,0);}}


theta_mu_postM->SaveAs(Form("comparison_RDMC/%s_theta_mu_post_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

theta_e_postM->SaveAs(Form("comparison_RDMC/%s_theta_e_post_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));


h_2dMerged->SaveAs(Form("comparison_RDMC/%s_2D_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

theta_muMerged->SaveAs(Form("comparison_RDMC/%s_theta_mu_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

theta_eMerged->SaveAs(Form("comparison_RDMC/%s_theta_e_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

d_acoM->SaveAs(Form("comparison_RDMC/%s_d_aco_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

h_openingM->SaveAs(Form("comparison_RDMC/%s_opening_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

h_z_posM->SaveAs(Form("comparison_RDMC/%s_z_post_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

h_nstubs_pre_s1M->SaveAs(Form("comparison_RDMC/%s_nstubs_pre_s1_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));



h_2d_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_2D_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

theta_mu_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_theta_mu_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

theta_e_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_theta_e_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

d_aco_preM->SaveAs(Form("comparison_RDMC/%s_preCuts_d_aco_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

h_opening_preM->SaveAs(Form("comparison_RDMC/%s_preCuts_opening_RD_parallel_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

h_z_pos_preM->SaveAs(Form("comparison_RDMC/%s_z_pre_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

h_nstubs_s1M->SaveAs(Form("comparison_RDMC/%s_nstubs_s1_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));


theta_RM->SaveAs(Form("comparison_RDMC/%s_thetaRight_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
theta_LM->SaveAs(Form("comparison_RDMC/%s_thetaLeft_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
theta_RLM->SaveAs(Form("comparison_RDMC/%s_thetaRvsThetaL_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
...
h_z_posM->Draw("hist");
a.SaveAs(Form("comparison_RDMC/%s_data_pos_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
*/


/*h_dist_mod2_range0M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_0_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
h_dist_mod2_range1M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_1_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
h_dist_mod2_range2M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_2_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
//h_dist_mod2_range3M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_3_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
//h_dist_mod2_range4M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_4_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
//h_dist_mod2_range5M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_5_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

h_dist_mod0_range0M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_0_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
h_dist_mod0_range1M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_1_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
h_dist_mod0_range2M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_2_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
//h_dist_mod0_range3M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_3_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
//h_dist_mod0_range4M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_4_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
//h_dist_mod0_range5M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_5_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
*/
...
c.SaveAs(Form("comparison_RDMC/%s_h_dist_ranges_el_mu_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
...
p.SaveAs(Form("comparison_RDMC/%s_pos_MOD2_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
*/
...
d.cd(1);
h_dist0M->Draw("hist");
h_dist0M->SaveAs(Form("comparison_RDMC/%s_distance_MOD0_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
d.cd(2);
h_dist1M->Draw("hist");
h_dist1M->SaveAs(Form("comparison_RDMC/%s_distance_MOD1_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
d.cd(3);
h_dist2M->Draw("hist");
h_dist2M->SaveAs(Form("comparison_RDMC/%s_distance_MOD2_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
d.cd(4);
h_dist3M->Draw("hist");
h_dist3M->SaveAs(Form("comparison_RDMC/%s_distance_MOD3_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
d.cd(5);
h_dist4M->Draw("hist");
h_dist4M->SaveAs(Form("comparison_RDMC/%s_distance_MOD4_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
d.cd(6);
h_dist5M->Draw("hist");
h_dist5M->SaveAs(Form("comparison_RDMC/%s_distance_MOD5_basicSelection_el_%s_%dhit_%s%sroot",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));

d.SaveAs(Form("comparison_RDMC/%s_distance_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
...
ad.cd(2);
h_dv_acoM->Draw("COLZ");
ad.SaveAs(Form("comparison_RDMC/%s_dist_VS_aco_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
...
n1.SaveAs(Form("comparison_RDMC/%s_distance_stubs_zoom_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
...
ns.SaveAs(Form("comparison_RDMC/%s_nstubs_track_basicSelection_el_%s_%dhit_%s%spdf",version.c_str(),bend.c_str(),nhits,mc.c_str(),sm.c_str()));
