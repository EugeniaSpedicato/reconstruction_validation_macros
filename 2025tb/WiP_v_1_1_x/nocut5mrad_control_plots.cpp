...
int hits = std::stoi(argv[3]);
    ...
    for (auto& entry : fs::directory_iterator(folder)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".root" and entry.path().string().find("nocut5mrad") != std::string::npos and entry.path().string().find("possible") == std::string::npos and entry.path().string().find("preVrtx") == std::string::npos and entry.path().string().find("smearing") == std::string::npos) {
...
std::cout<<"h_x_1.size() "<< h_x_1.size() <<std::endl;
...
pos.SaveAs(Form("plots/run%i/nocut5mrad_preFid_postion_nhits%i.pdf",run,hits));
...
gen.SaveAs(Form("plots/run%i/nocut5mrad_postFid_nhits%i.pdf",run,hits));
...
pe.SaveAs(Form("plots/run%i/nocut5mrad_preEl_nhits%i.pdf",run,hits));
pe.SaveAs(Form("plots/run%i/nocut5mrad_preEl_nhits%i.root",run,hits));
...
fe.SaveAs(Form("plots/run%i/nocut5mrad_finalEl_nhits%i.pdf",run,hits));
...
gen_fin.SaveAs(Form("plots/run%i/nocut5mrad_final_nhits%i.pdf",run,hits));
gen_fin.SaveAs(Form("plots/run%i/nocut5mrad_final_nhits%i.root",run,hits));


auto f1 = new TF1("f1","[0]+[1]*x*x",-0.002,0.002);
auto f2 = new TF1("f2","[0]+[1]*x*x",-0.002,0.002);

TCanvas umb("umb","umb",1000,500);
umb.Divide(2,1);
umb.cd(1);
h_final_rndm_0.at(0)->Fit("f1","R"," ",-0.002,0.002);
h_final_rndm_0.at(0)->Draw();
umb.cd(2);
h_final_rndm_1.at(0)->Fit("f2","R"," ",-0.002,0.002);
h_final_rndm_1.at(0)->Draw();
umb.SaveAs(Form("plots/run%i/nocut5mrad_final_umb_nhits%i.root",run,hits));
