...
int hits = std::stoi(argv[3]);
    ...
    for (auto& entry : fs::directory_iterator(folder)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".root" and entry.path().string().find("wip11x") != std::string::npos and entry.path().string().find("possible") == std::string::npos and entry.path().string().find("preVrtx") == std::string::npos and entry.path().string().find("smearing") == std::string::npos) {
...
pos.SaveAs(Form("plots/run%i/wip11x_preFid_postion_nhits%i.pdf",run,hits));
...
gen.SaveAs(Form("plots/run%i/wip11x_postFid_nhits%i.pdf",run,hits));
...
pe.SaveAs(Form("plots/run%i/wip11x_preEl_nhits%i.pdf",run,hits));
pe.SaveAs(Form("plots/run%i/wip11x_preEl_nhits%i.root",run,hits));
...
fe.SaveAs(Form("plots/run%i/wip11x_finalEl_nhits%i.pdf",run,hits));
h_E_th_min_0.at(0)->SaveAs(Form("plots/run%i/thetas/wip11x_thetaMin0_nhits%i.root",run,hits));
h_E_th_min_1.at(0)->SaveAs(Form("plots/run%i/thetas/wip11x_thetaMin1_nhits%i.root",run,hits));
h_E_th_max_0.at(0)->SaveAs(Form("plots/run%i/thetas/wip11x_thetaMax0_nhits%i.root",run,hits));
h_E_th_max_1.at(0)->SaveAs(Form("plots/run%i/thetas/wip11x_thetaMax1_nhits%i.root",run,hits));
...
gen_fin.SaveAs(Form("plots/run%i/wip11x_final_nhits%i.pdf",run,hits));
gen_fin.SaveAs(Form("plots/run%i/wip11x_final_nhits%i.root",run,hits));
...
umb.SaveAs(Form("plots/run%i/wip11x_final_umb_nhits%i.root",run,hits));
*/
