...
int smearing_report_count_snakemake(int run, string type, string filen, int nhits){
...
double smearing1x=3.e-06;
double smearing2x=3.e-06;
double smearing3x=0.;
double smearing1y=0.;
double smearing2y=0.;
double smearing3y=0.;
...
if(trig_bits.onlineTriggerSingleMuonInteraction0()==1 and nTr.at(0)==1 and abs(muin_4.position())<=1.4985 and abs(muin_5.position())<=1.4985 and stub0<10){
	...
	//double thmu_rec=vrtx.at(0)->muonTheta();
		...
		double IPx=pos_on_track(track_vrtx1.at(1).x0(),track_vrtx1.at(1).xSlope()+gRandom->Gaus(0.,smearing1x),(667.3-2.7)) - pos_on_track(track_vrtx1.at(0).x0(),track_vrtx1.at(0).xSlope()+gRandom->Gaus(0.,smearing1x),(667.3-2.7));
                double IPy=pos_on_track(track_vrtx1.at(1).y0(),track_vrtx1.at(1).ySlope()+gRandom->Gaus(0.,smearing1y),(667.3-2.7)) - pos_on_track(track_vrtx1.at(0).y0(),track_vrtx1.at(0).ySlope()+gRandom->Gaus(0.,smearing1y),(667.3-2.7));

                IP_out=sqrt(IPx*IPx + IPy*IPy);
                h_IP_0->Fill(IP_out);

		TVector3 p_e(vrtx.at(0)->outgoingElectron().xSlope()+gRandom->Gaus(0.,smearing2x),vrtx.at(0)->outgoingElectron().ySlope()+gRandom->Gaus(0.,smearing2y),1.0); p_e.Unit();
		TVector3 p_mu(vrtx.at(0)->outgoingMuon().xSlope()+gRandom->Gaus(0.,smearing2x),vrtx.at(0)->outgoingMuon().ySlope()+gRandom->Gaus(0.,smearing2y),1.0); p_mu.Unit();
		TVector3 p_mu_in(vrtx.at(0)->incomingMuon().xSlope()+gRandom->Gaus(0.,smearing1x),vrtx.at(0)->incomingMuon().ySlope()+gRandom->Gaus(0.,smearing1y),1.0); p_mu_in.Unit();
...
//	double thmu_rec=vrtx.at(0)->muonTheta();
	...
	double zpos =vrtx.at(0)->zPositionFit();
		...
		double IPx=pos_on_track(track_vrtx2.at(1).x0(),track_vrtx2.at(1).xSlope()+gRandom->Gaus(0.,smearing2x),(784.6-3.9)) - pos_on_track(track_vrtx2.at(0).x0(),track_vrtx2.at(0).xSlope()+gRandom->Gaus(0.,smearing2x),(784.6-3.9));
                double IPy=pos_on_track(track_vrtx2.at(1).y0(),track_vrtx2.at(1).ySlope()+gRandom->Gaus(0.,smearing2y),(784.6-3.9)) - pos_on_track(track_vrtx2.at(0).y0(),track_vrtx2.at(0).ySlope()+gRandom->Gaus(0.,smearing2x),(784.6-3.9));

                IP_out=sqrt(IPx*IPx + IPy*IPy);
                h_IP_1->Fill(IP_out);

                TVector3 p_e(vrtx.at(0)->outgoingElectron().xSlope()+gRandom->Gaus(0.,smearing3x),vrtx.at(0)->outgoingElectron().ySlope()+gRandom->Gaus(0.,smearing3y),1.0); p_e.Unit();
                TVector3 p_mu(vrtx.at(0)->outgoingMuon().xSlope()+gRandom->Gaus(0.,smearing3x),vrtx.at(0)->outgoingMuon().ySlope()+gRandom->Gaus(0.,smearing3y),1.0); p_mu.Unit();
		TVector3 p_mu_in(vrtx.at(0)->incomingMuon().xSlope()+gRandom->Gaus(0.,smearing2x),vrtx.at(0)->incomingMuon().ySlope()+gRandom->Gaus(0.,smearing2y),1.0); p_mu_in.Unit();
...
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;
    ...
    std::ofstream out(Form("txt/run%i/smearing_report_%s_%s_nhits%i.txt",run,type.c_str(),filen.c_str(),nhits)); // apre (o crea) il file in scrittura
    if (!out) {
    ...
    TFile out_root(Form("txt/run%i/smearing_root_%s_%s_nhits%i.root",run,type.c_str(),filen.c_str(),nhits),"recreate");
