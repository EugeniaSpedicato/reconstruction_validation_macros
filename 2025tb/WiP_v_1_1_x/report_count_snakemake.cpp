...
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;
    ...
    std::ofstream out(Form("txt/run%i/wip11x_report_%s_%s_nhits%i.txt",run,type.c_str(),filen.c_str(),nhits)); // apre (o crea) il file in scrittura
    if (!out) {
    ...
    TFile out_root(Form("txt/run%i/wip11x_root_%s_%s_nhits%i.root",run,type.c_str(),filen.c_str(),nhits),"recreate");
