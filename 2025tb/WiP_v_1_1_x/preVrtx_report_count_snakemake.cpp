...
int preVrtx_report_count_snakemake(int run, string type, string filen, int nhits){
		  ...
		  h_el_1->Fill(the_rec,thmu_rec);
			...
			if(thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002){ //if(thmu_rec>0.0002 and thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002){
				...
				h_final_aco_0->Fill(acoplanarity_v);
                                ...
                                h_final_chi2_trMax_0->Fill(track_e.chi2());
				h_final_chi2_trMin_0->Fill(track_mu.chi2());
				//h_el_1->Fill(the_rec,thmu_rec);
				}
		  ...
		  h_elastic_1->Fill(Elastic2-thmu_rec);
				h_el_2->Fill(the_rec,thmu_rec);
                        ...
                        if(thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002){ //if(thmu_rec>0.0002 and thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002){
                                ...
                                h_final_chi2_trMax_1->Fill(track_e.chi2());
                                h_final_chi2_trMin_1->Fill(track_mu.chi2());
				//h_el_2->Fill(the_rec,thmu_rec);
				}
                        }
		}
...
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;
    ...
    std::ofstream out(Form("txt/run%i/preVrtx_report_%s_%s_nhits%i.txt",run,type.c_str(),filen.c_str(),nhits)); // apre (o crea) il file in scrittura
    if (!out) {
    ...
    TFile out_root(Form("txt/run%i/preVrtx_root_%s_%s_nhits%i.root",run,type.c_str(),filen.c_str(),nhits),"recreate");
