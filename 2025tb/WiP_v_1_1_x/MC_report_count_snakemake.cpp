...
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;
    ...
    std::ofstream out(Form("txt/run%s/wip11x_report_%s.txt",run.c_str(),type.c_str())); // apre (o crea) il file in scrittura
    if (!out) {
    ...
    TFile out_root(Form("txt/run%s/wip11x_root_%s.root",run.c_str(),type.c_str()),"recreate");
