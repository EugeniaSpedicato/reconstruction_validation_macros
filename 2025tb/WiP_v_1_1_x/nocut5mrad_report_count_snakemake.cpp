...
int nocut5mrad_report_count_snakemake(int run, string type, string filen, int nhits){
...
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;
    ...
    std::ofstream out(Form("txt/run%i/nocut5mrad_report_%s_%s_nhits%i.txt",run,type.c_str(),filen.c_str(),nhits)); // apre (o crea) il file in scrittura
    if (!out) {
    ...
    TFile out_root(Form("txt/run%i/nocut5mrad_root_%s_%s_nhits%i.root",run,type.c_str(),filen.c_str(),nhits),"recreate");
