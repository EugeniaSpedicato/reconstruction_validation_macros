...
void MC_effRecoEcal(void) {
TChain chain("cbmsim");  // replace "myTree" with your TTree name

chain.Add("/eos/user/e/ehess/production_Mesmer/FairMUonE_WiP_v1.1.x/Condor_files/MCsignalJob_target1_2.root");
chain.Add("/eos/user/e/ehess/production_Mesmer/FairMUonE_WiP_v1.1.x/Condor_files/MCsignalJob_target1_3.root");
chain.Add("/eos/user/e/ehess/production_Mesmer/FairMUonE_WiP_v1.1.x/Condor_files/MCsignalJob_target1_4.root");
...
//    double electronDeltaYfromCaloCentroid =  ReconstructionOutput->reconstructedCalorimeterCluster().yCentroid();
   ...
   //
    ...
    if( nTr.at(1) != 1 ||  nTr.at(2) != 2) continue;

    hEreco12->Fill(clusterEnergy,wgt_full);
   //
   // elastic selection variables
   //
   double dotProduct_v = p_v2.at(0).Dot(p_v2.at(1));
   TVector3 crossProduct_v = p_v2.at(0).Cross(p_v2.at(1));
   double T_v = p_v1.at(0).Dot(crossProduct_v);
   TVector3 i1_v = p_v1.at(0).Cross(p_v2.at(0));
   TVector3 i2_v = p_v1.at(0).Cross(p_v2.at(1));
   T_v = T_v > 0 ? 1 : -1;
   ...
   //
   double theta1 = p_v1.at(0).Angle(p_v2.at(0));
   double theta2 = p_v1.at(0).Angle(p_v2.at(1));
...
//       if(theta1+theta2 < 4.E-3) continue;
   ...
   hrel->SetMarkerSize(0.5);
      ...
      TString filename = "effRecoT1ECAL30GeV.root";
