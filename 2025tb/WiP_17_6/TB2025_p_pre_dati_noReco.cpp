...
TVector3 p_e,p_mu,p_muin;
        ...
        std::vector<Short_t> hits_mu=vrtx->outgoingMuon().hitIds();
	std::vector<MUonERecoOutputHitAnalysis> v_hits_mu;
        std::vector<double> pos_mu; pos_mu.resize(6);
        std::vector<Short_t> hits_e=vrtx->outgoingElectron().hitIds();
...
m.cd(2);
mod_elM->Draw("hist");
m.SaveAs(Form("comparison_RDMC/%s_module_tracks_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
