...
if(trig_bits.onlineTriggerSingleMuonInteraction0()==1 and nTr.at(0)==1 and abs(muin_4.position())<=1.4985 and abs(muin_5.position())<=1.4985 and stub0<10){
	...
	//double thmu_rec=vrtx->muonTheta();
		...
		TVector3 p_e(vrtx->outgoingElectron().xSlope()+gRandom->Gaus(0.,smearing2x),vrtx->outgoingElectron().ySlope()+gRandom->Gaus(0.,smearing2y),1.0); p_e.Unit();
		TVector3 p_mu(vrtx->outgoingMuon().xSlope()+gRandom->Gaus(0.,smearing2x),vrtx->outgoingMuon().ySlope()+gRandom->Gaus(0.,smearing2y),1.0); p_mu.Unit();
		TVector3 p_mu_in(vrtx->incomingMuon().xSlope()+gRandom->Gaus(0.,smearing1x),vrtx->incomingMuon().ySlope()+gRandom->Gaus(0.,smearing1y),1.0); p_mu_in.Unit();
...
//	double thmu_rec=vrtx->muonTheta();
	...
	double zpos =vrtx->zPositionFit();
		...
		double IPx=pos_on_track(track_vrtx2.at(1).x0(),track_vrtx2.at(1).xSlope()+gRandom->Gaus(0.,smearing2x),(784.6-3.9)) - pos_on_track(track_vrtx2.at(0).x0(),track_vrtx2.at(0).xSlope()+gRandom->Gaus(0.,smearing2x),(784.6-3.9));
                ...
                TVector3 p_e(vrtx->outgoingElectron().xSlope()+gRandom->Gaus(0.,smearing3x),vrtx->outgoingElectron().ySlope()+gRandom->Gaus(0.,smearing3y),1.0); p_e.Unit();
                TVector3 p_mu(vrtx->outgoingMuon().xSlope()+gRandom->Gaus(0.,smearing3x),vrtx->outgoingMuon().ySlope()+gRandom->Gaus(0.,smearing3y),1.0); p_mu.Unit();
		TVector3 p_mu_in(vrtx->incomingMuon().xSlope()+gRandom->Gaus(0.,smearing2x),vrtx->incomingMuon().ySlope()+gRandom->Gaus(0.,smearing2y),1.0); p_mu_in.Unit();
