...
int MC_report_count_snakemake(string run, string type){
...
/*
cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/TB2025/run%s/%s/%s_%ihit_WiP_17_6.root",run.c_str(),type.c_str(),filen.c_str(),nhits));
cout << Form("/mnt/raid10/DATA/espedica/fairmu/TB2025/run%s/%s/%s_%ihit_WiP_17_6.root",run.c_str(),type.c_str(),filen.c_str(),nhits) << endl;
*/
//cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/TB2025/MC/ideal_%s_noECAL_WiP_v1_1_x.root",type.c_str()));
cbmsim->Add("/eos/user/e/espedica/mesmer25/MCsignalJob_target2_1.root");
...
if(type=="single_int_0")count_tr_1->Fill(1);
if(type=="single_int_1")count_tr_2->Fill(1);
...
if(type=="single_int_0" and nTr.at(0)==1){
...
else if(type=="single_int_1" and nTr.at(1)==1){
...
if(type=="single_int_0" and nTr.at(0)==1 and abs(muin_4.position())<=1.4985 and abs(muin_5.position())<=1.4985 and stub0<10){
...
//	if(isReco==0)std::cout<<"isR entry --? " << entry << " ?--" << std::endl;
...
else if(type=="single_int_1" and nTr.at(1)==1 and abs(muin_4.position())<=1.4985 and abs(muin_5.position())<=1.4985 and stub1<10){
...
//h_nvrtx_2->Fill(1);
	if(chi<20) {good_vrtx_2->Fill(1,wgt); if(chi>2)std::cout<<"entry " << entry << " chi " << chi << std::endl;}
	double the_rec=vrtx->electronTheta();
	double thmu_rec=vrtx->muonTheta();
	double acoplanarity_v=vrtx->modifiedAcoplanarity();
	double zpos =vrtx->zPositionFit();
...
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;
    ...
    std::ofstream out(Form("txt/run%s/emma_report_%s.txt",run.c_str(),type.c_str())); // apre (o crea) il file in scrittura
    if (!out) {
    ...
    TFile out_root(Form("txt/run%s/emma_root_%s.root",run.c_str(),type.c_str()),"recreate");
      if(type=="single_int_0"){
	h_x_0M->Write();
	h_y_0M->Write();
	h_thx_0M->Write();
	h_thy_0M->Write();
