...
int MC_rndm_preVrtx_report_count_snakemake(string run, string type){
...
/*
cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/TB2025/run%s/%s/%s_%ihit_WiP_17_6.root",run.c_str(),type.c_str()));
cout << Form("/mnt/raid10/DATA/espedica/fairmu/TB2025/run%s/%s/%s_%ihit_WiP_17_6.root",run.c_str(),type.c_str()) << endl;
*/
...
ROOT::TTreeProcessorMT tp1(*cbmsim,nthreads);
   ...
   ROOT::TThreadedObject<TH2D> h_el_1("h_el_1","h_el_1",320, 0.,0.032,320, 0.,0.032);
   ROOT::TThreadedObject<TH2D> h_el_2("h_el_2","h_el_2",320, 0.,0.032,320, 0.,0.032);
   ...
   ROOT::TThreadedObject<TH1D> h_th_min_0("h_th_min_0","theta min tar 0",160,0.,0.032);
   ROOT::TThreadedObject<TH1D> h_th_max_0("h_th_max_0","theta max tar 0",160,0.,0.032);
   ROOT::TThreadedObject<TH1D> h_opening_0("h_opening_0","opening angle tar 0",160,0.,0.032);

   ROOT::TThreadedObject<TH1D> h_preE_th_min_0("h_preE_th_min_0","theta min tar 0 PreEl",160,0.,0.032);
   ...
   ROOT::TThreadedObject<TH1D> h_E_th_min_0("h_E_th_min_0","theta min tar 0 Elastic cut",160,0.,0.032);
   ...
   ROOT::TThreadedObject<TH1D> h_th_min_1("h_th_min_1","theta min tar 1",160,0.,0.032);
   ROOT::TThreadedObject<TH1D> h_th_max_1("h_th_max_1","theta max tar 1",160,0.,0.032);
   ROOT::TThreadedObject<TH1D> h_opening_1("h_opening_1","opening tar 1",160,0.,0.032);

   ROOT::TThreadedObject<TH1D> h_preEl_th_min_1("h_preE_th_min_1","theta min tar 1 PreEl",160,0.,0.032);
   ...
   ROOT::TThreadedObject<TH1D> h_E_th_min_1("h_E_th_min_1","theta min tar 1 Elastic cut",160,0.,0.032);
...
MUonERecoOutputTrackAnalysis track_2;
MUonERecoOutputTrackAnalysis track_1;
        ...
        if(track.sector()==0){nTr.at(0)++; track_0.push_back(track);TVector3 p(track.xSlope(),track.ySlope(),1.0);p.Unit(); p_v0.push_back(p);}
                ...
                TVector3 p(track.xSlope(),track.ySlope(),1.0); p.Unit(); p_v1.push_back(p);
				   //if(track.index()==mu_out_v.index()){track_vrtx1.push_back(track);track_2=track;}
				   //else if(track.index()==e_out_v.index()){track_vrtx1.push_back(track);track_1=track;}
		}
                ...
                TVector3 p(track.xSlope(),track.ySlope(),1.0); p.Unit(); p_v2.push_back(p);
                                   //if(track.index()==mu_out_v.index()){track_vrtx2.push_back(track);track_2=track;}
                                   //else if(track.index()==e_out_v.index()){track_vrtx2.push_back(track);track_1=track;}
		}
        }
...
//	if(chi!=0 and vrtx.at(0).stationIndex()==1){
	...
	good_vrtx_1->Fill(1,wgt);

        double th1=0.;
        double th2=0.;
if(entry%2==0){track_1=track_vrtx1.at(0);track_2=track_vrtx1.at(1);}
else{track_1=track_vrtx1.at(1);track_2=track_vrtx1.at(0);}
		TVector3 p_mu_in(track_0.at(0).xSlope(),track_0.at(0).ySlope(),1.0); p_mu_in.Unit();
		TVector3 p_e(track_1.xSlope(),track_1.ySlope(),1.0); p_e.Unit();
		TVector3 p_mu(track_2.xSlope(),track_2.ySlope(),1.0); p_mu.Unit();


	th1=p_mu_in.Angle(p_e);
	th2=p_mu_in.Angle(p_mu);
	...
	double zpos =0;
		...
		if(IP_out<0.2){
		 ...
		 h_chi2_vrtx_0->Fill(1,wgt);
		 h_Dz_vrtx_0->Fill(667.3-2.7-(667.3-2.7),wgt);
		 h_aco_0->Fill(acoplanarity_v,wgt);
		 h_th_min_0->Fill(th2,wgt);
		 h_th_max_0->Fill(th1,wgt);

		 if(stub1<=15 and abs(acoplanarity_v)<=0.3 and abs(667.3-2.7-(667.3-2.7))<3. and th1<0.02 and th2>0.0002 and th2<0.02 and th1>0.0002){


	          pre_elastic_1->Fill(1,wgt);

		  h_preE_th_min_0->Fill(th2,wgt);
		  h_preE_th_max_0->Fill(th1,wgt);
		  h_elastic_0->Fill(th2,wgt);
		  h_el_1->Fill(th1,th2,wgt);

			        elastic_1->Fill(1,wgt);
	        	        h_E_th_min_0->Fill(th2,wgt);
		                h_E_th_max_0->Fill(th1,wgt);
                                h_final_nhits_pre_0->Fill(stub0,wgt);
                                h_final_nhits_post_0->Fill(stub1,wgt);
                                h_final_ntracks_post_0->Fill(nTr.at(1),wgt);
                                h_final_chi2_vrtx_0->Fill(1,wgt);
                                h_final_Dz_vrtx_0->Fill(667.3-2.7-(667.3-2.7),wgt);
				h_final_aco_0->Fill(acoplanarity_v,wgt);
                                h_final_chi2_tr0_0->Fill(track_0.at(0).chi2(),wgt);
                                h_final_chi2_trMax_0->Fill(track_1.chi2(),wgt);
				h_final_chi2_trMin_0->Fill(track_2.chi2(),wgt);
				h_opening_0->Fill(p_mu.Angle(p_e),wgt);
				//h_el_1->Fill(th1,th2);
	...
	//if(chi!=0 and vrtx.at(0).stationIndex()==2){
	 if(nTr.at(2)>1){
//h_nvrtx_2->Fill(1);
	good_vrtx_2->Fill(1,wgt);

if(entry%2==0){track_1=track_vrtx2.at(0);track_2=track_vrtx2.at(1);}
else{track_1=track_vrtx2.at(1);track_2=track_vrtx2.at(0);}

                TVector3 p_mu_in(track_vrtx1.at(0).xSlope(),track_vrtx1.at(0).ySlope(),1.0); p_mu_in.Unit();
                TVector3 p_e(track_1.xSlope(),track_1.ySlope(),1.0); p_e.Unit();
                TVector3 p_mu(track_2.xSlope(),track_2.ySlope(),1.0); p_mu.Unit();

        double th1=p_mu_in.Angle(p_e);
        double th2=p_mu_in.Angle(p_mu);
	...
	h_nvrtx_2->Fill(1,wgt);
		 ...
		 h_chi2_vrtx_1->Fill(1,wgt);
		 h_Dz_vrtx_1->Fill(784.6-3.9-(784.6-3.9),wgt);
		 h_aco_1->Fill(acoplanarity_v,wgt);
		 h_th_min_1->Fill(th2,wgt);
		 h_th_max_1->Fill(th1,wgt);

		 if(stub2<=15 and abs(acoplanarity_v)<=0.3 and abs(784.6-3.9-(784.6-3.9))<3. and th1<0.02 and th2>0.0002 and th2<0.02 and th1>0.0002){
		  pre_elastic_2->Fill(1,wgt);
		  h_preEl_th_min_1->Fill(th2,wgt);
		  h_preE_th_max_1->Fill(th1,wgt);
		  h_elastic_1->Fill(th2,wgt);
				h_el_2->Fill(th1,th2,wgt);

                                elastic_2->Fill(1,wgt);
                                h_E_th_min_1->Fill(th2,wgt);
                                h_E_th_max_1->Fill(th1,wgt);
                                h_final_nhits_pre_1->Fill(stub1,wgt);
                                h_final_nhits_post_1->Fill(stub2,wgt);
                                h_final_ntracks_post_1->Fill(nTr.at(2),wgt);
                                h_final_chi2_vrtx_1->Fill(1,wgt);
                                h_final_Dz_vrtx_1->Fill(784.6-3.9-(784.6-3.9),wgt);
                                h_final_aco_1->Fill(acoplanarity_v,wgt);
                                h_final_chi2_tr0_1->Fill(track_vrtx1.at(0).chi2(),wgt);
                                h_final_chi2_trMax_1->Fill(track_1.chi2(),wgt);
                                h_final_chi2_trMin_1->Fill(track_2.chi2(),wgt);
                                h_opening_1->Fill(p_mu.Angle(p_e),wgt);
				//h_el_2->Fill(th1,th2);
                        }
		}
	}
  ...
  auto h_opening_0M=h_opening_0.Merge();
  ...
  auto h_opening_1M=h_opening_1.Merge();

//cout << "good_vrtx_1->Integral() " << good_vrtx_1->Integral() << endl;
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;

    std::ofstream out(Form("txt/run%s/rndm_preVrtx_report_%s.txt",run.c_str(),type.c_str())); // apre (o crea) il file in scrittura
    if (!out) {
    ...
    TFile out_root(Form("txt/run%s/rndm_preVrtx_root_%s.root",run.c_str(),type.c_str()),"recreate");
      if(type=="single_muon_interaction_0"){
	h_x_0M->Write();
	h_y_0M->Write();
	h_thx_0M->Write();
