...
int sec_MC_preVrtx_report_count_snakemake(string run, string type){
...
if(type=="single_int_0")count_tr_1->Fill(1,wgt);
if(type=="single_int_1")count_tr_2->Fill(1,wgt);
...
//cout << "fiducial_1->Integral() " << fiducial_1->Integral() << endl;
    ...
    TFile out_root(Form("txt/run%s/preVrtx_root_%s.root",run.c_str(),type.c_str()),"recreate");
