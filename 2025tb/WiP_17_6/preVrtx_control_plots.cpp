...
int hits = std::stoi(argv[3]);
    ...
    for (auto& entry : fs::directory_iterator(folder)) {
        if (fs::is_regular_file(entry.path()) && entry.path().extension() == ".root" and entry.path().string().find("preVrtx") != std::string::npos) {
...
pos.SaveAs(Form("plots/run%i/preVrtx_preFid_postion_nhits%i.pdf",run,hits));
...
gen.SaveAs(Form("plots/run%i/preVrtx_postFid_nhits%i.pdf",run,hits));
...
pe.SaveAs(Form("plots/run%i/preVrtx_preEl_nhits%i.pdf",run,hits));
...
fe.SaveAs(Form("plots/run%i/preVrtx_finalEl_nhits%i.pdf",run,hits));
...
gen_fin.SaveAs(Form("plots/run%i/preVrtx_final_nhits%i.pdf",run,hits));
gen_fin.SaveAs(Form("plots/run%i/preVrtx_final_nhits%i.root",run,hits));
