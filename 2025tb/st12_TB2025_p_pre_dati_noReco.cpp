...
int st12_TB2025_p_pre_dati_noReco(string version, int nhits,string bend,string mc,string save){
...
if(version=="tb25_dp_int1" and nhits==0 and bend=="Bend"){
...
else if(version=="tb25_run11_unb_st12" and nhits==0 and bend=="Bend"){
...
else if(version=="tb25_int1_run11" and nhits==0 and bend=="Bend"){
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run11/single_muon_interaction_1/muedaq01-1750458088_0hit.root");
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run11/single_muon_interaction_1/muedaq01-1750461265_0hit.root");
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run11/single_muon_interaction_1/muedaq01-1750461334_0hit.root");
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run11/single_muon_interaction_1/muedaq01-1750461422_0hit.root");
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run11/single_muon_interaction_1/muedaq01-1750461710_0hit.root");
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run11/single_muon_interaction_1/muedaq01-1750466914_0hit.root");
}
else if(version=="tb25_int1_unb_run11" and nhits==0 and bend=="Bend"){
...
else if(version=="minbias_int1" and nhits==0 and bend=="Bend"){
...
else if(version=="run20_int1_newAl2" and nhits==0 and bend=="Bend"){
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run20/single_muon_interaction_1/4files_commit62871d8b_newAl2.root");
}
else if(version=="run48_int1" and nhits==0){
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run48/single_muon_interaction_1/3files_noMuPid.root");
}
...
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run24/single_muon_interaction_1/1file_0hit_noMuPid.root");
}
else if(version=="run25_mark" and nhits==0){
cbmsim->Add("/mnt/raid10/DATA/espedica/fairmu/TB2025/run25/single_muon_interaction_1/1file_0hit_noMuPid.root");
}
...
TH1::SetDefaultSumw2(kTRUE);
   ...
   ROOT::TThreadedObject<TH1D> h_z_pos_pre("h_z_pos_pre","Z selected events pos fit pre-selection",800, 500.,900.);
   ROOT::TThreadedObject<TH1D> h_z_pos("h_z_pos","Z selected events pos fit",800, 500.,900.);
...
*/
   ...
   ROOT::TThreadedObject<TH1D> fiducial("fiducial","fiducial",2,0,2);
   ROOT::TThreadedObject<TH1D> count_chi("fiducial","fiducial",2,0,2);

   ROOT::TThreadedObject<TH1D> elastic("elastic","elastic",2,0,2);
...
double z_fix=912.7;
 ...
 double chi=0;
if(vrtx->stationIndex()==2) chi=vrtx->chi2perDegreeOfFreedom();
...
if(chi>0 and stub1<=15 and thmu_rec<=Elastic2+0.0002 and thmu_rec>=Elastic2-0.0002 and abs(acoplanarity_v)<=0.4 and zpos>650){
...
if(t_e.xSlope()>t_mu.xSlope()){theta_R->Fill(the_rec); theta_L->Fill(thmu_rec); theta_RL->Fill(the_rec,thmu_rec); }
...
std::array<double,6> t_e_hits;t_e_hits.fill({-99.});
 ...
 for(int h=0; h<t_mu.hits().size(); h++){
  if(t_mu.hits().at(h).stationID()==2) {mod_mu->Fill(t_mu.hits().at(h).moduleID()); t_mu_hits.at(t_mu.hits().at(h).moduleID())=t_mu.hits().at(h).position();}
 }

 for(int h=0; h<t_e.hits().size(); h++){
  if(t_e.hits().at(h).stationID()==2) {mod_el->Fill(t_e.hits().at(h).moduleID()); t_e_hits.at(t_e.hits().at(h).moduleID())=t_e.hits().at(h).position();}
 }
...
auto fiducialM=fiducial.Merge();
auto elasticM=elastic.Merge();
auto count_chiM=count_chi.Merge();
...
cout <<"Events with reco vrtx " << count_chiM->Integral() << endl;
cout <<"Events passing fiducial " << fiducialM->Integral() << endl;
cout <<"Events passing fiducial and elastic " << elasticM->Integral() << endl;


if(save=="yes"){
vrtx_chi2M->SaveAs(Form("comparison_RDMC/%s_vrtx_chi2_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
track_elM->SaveAs(Form("comparison_RDMC/%s_track_el_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
track_muM->SaveAs(Form("comparison_RDMC/%s_track_mu_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
...
d.cd(1);
h_dist0M->Draw("hist");
h_dist0M->SaveAs(Form("comparison_RDMC/%s_distance_MOD0_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
d.cd(2);
h_dist1M->Draw("hist");
h_dist1M->SaveAs(Form("comparison_RDMC/%s_distance_MOD1_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
d.cd(3);
h_dist2M->Draw("hist");
h_dist2M->SaveAs(Form("comparison_RDMC/%s_distance_MOD2_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
d.cd(4);
h_dist3M->Draw("hist");
h_dist3M->SaveAs(Form("comparison_RDMC/%s_distance_MOD3_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
d.cd(5);
h_dist4M->Draw("hist");
h_dist4M->SaveAs(Form("comparison_RDMC/%s_distance_MOD4_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
d.cd(6);
h_dist5M->Draw("hist");
h_dist5M->SaveAs(Form("comparison_RDMC/%s_distance_MOD5_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

d.SaveAs(Form("comparison_RDMC/%s_distance_basicSelection_%s_%dhit_%s.pdf",version.c_str(),bend.c_str(),nhits,mc.c_str()));


theta_mu_postM->SaveAs(Form("comparison_RDMC/%s_theta_mu_post_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

theta_e_postM->SaveAs(Form("comparison_RDMC/%s_theta_e_post_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));


h_2dMerged->SaveAs(Form("comparison_RDMC/%s_2D_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

theta_muMerged->SaveAs(Form("comparison_RDMC/%s_theta_mu_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));


theta_eMerged->SaveAs(Form("comparison_RDMC/%s_theta_e_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

d_acoM->SaveAs(Form("comparison_RDMC/%s_d_aco_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

h_openingM->SaveAs(Form("comparison_RDMC/%s_opening_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

h_z_posM->SaveAs(Form("comparison_RDMC/%s_z_post_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

h_nstubs_pre_s1M->SaveAs(Form("comparison_RDMC/%s_nstubs_pre_s1_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));


h_2d_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_2D_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

theta_mu_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_theta_mu_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

theta_e_preMerged->SaveAs(Form("comparison_RDMC/%s_preCuts_theta_e_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

d_aco_preM->SaveAs(Form("comparison_RDMC/%s_preCuts_d_aco_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

h_opening_preM->SaveAs(Form("comparison_RDMC/%s_preCuts_opening_RD_parallel_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

h_z_pos_preM->SaveAs(Form("comparison_RDMC/%s_z_pre_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

h_nstubs_s1M->SaveAs(Form("comparison_RDMC/%s_nstubs_s1_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

theta_RM->SaveAs(Form("comparison_RDMC/%s_thetaRight_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
theta_LM->SaveAs(Form("comparison_RDMC/%s_thetaLeft_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
theta_RLM->SaveAs(Form("comparison_RDMC/%s_thetaRvsThetaL_basicSelection_el_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
...
h_z_posM->Draw("hist");
a.SaveAs(Form("comparison_RDMC/%s_data_pos_basicSelection_%s_%dhit_%s.pdf",version.c_str(),bend.c_str(),nhits,mc.c_str()));
*/


/*h_dist_mod2_range0M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_0_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
h_dist_mod2_range1M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_1_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
h_dist_mod2_range2M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_2_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
//h_dist_mod2_range3M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_3_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
//h_dist_mod2_range4M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_4_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
//h_dist_mod2_range5M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod2_range_5_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));

h_dist_mod0_range0M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_0_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
h_dist_mod0_range1M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_1_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
h_dist_mod0_range2M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_2_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
//h_dist_mod0_range3M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_3_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
//h_dist_mod0_range4M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_4_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
//h_dist_mod0_range5M->SaveAs(Form("comparison_RDMC/%s_h_dist_mod0_range_5_basicSelection_%s_%dhit_%s.root",version.c_str(),bend.c_str(),nhits,mc.c_str()));
*/
...
c.SaveAs(Form("comparison_RDMC/%s_h_dist_ranges_el_mu_basicSelection_%s_%dhit_%s.pdf",version.c_str(),bend.c_str(),nhits,mc.c_str()));
...
p.SaveAs(Form("comparison_RDMC/%s_pos_MOD2_basicSelection_%s_%dhit_%s.pdf",version.c_str(),bend.c_str(),nhits,mc.c_str()));
*/
...
ad.cd(2);
h_dv_acoM->Draw("COLZ");
ad.SaveAs(Form("comparison_RDMC/%s_dist_VS_aco_basicSelection_%s_%dhit_%s.pdf",version.c_str(),bend.c_str(),nhits,mc.c_str()));
...
n1.SaveAs(Form("comparison_RDMC/%s_distance_stubs_zoom_basicSelection_%s_%dhit_%s.pdf",version.c_str(),bend.c_str(),nhits,mc.c_str()));
...
ns.SaveAs(Form("comparison_RDMC/%s_nstubs_track_basicSelection_%s_%dhit_%s.pdf",version.c_str(),bend.c_str(),nhits,mc.c_str()));
}
