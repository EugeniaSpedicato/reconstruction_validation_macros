...
TH1D *h_resTR=new TH1D("resTR", "(the_rec-the_true) Ee<5 GeV POST-VRTX",100,-0.01,0.01);
TH1D *h_res1TR=new TH1D("res1TR", "(the_rec-the_true) 5<E<15 GeV POST-VRTX",50,-0.0025,0.0025);
TH1D *h_res2TR=new TH1D("res2TR", "(the_rec-the_true) 15<E<25 GeV POST-VRTX",40,-0.001,0.001);
TH1D *h_res3TR=new TH1D("res3TR", "(the_rec-the_true) Ee>25 GeV POST-VRTX",40,-0.001,0.001);

TH2D *h_res_muTR=new TH2D("h_res_muTR", "(thmu_rec-thmu_true) VS energy Emu(GeV) POST-VRTX",30,155,160,120,-0.0006,0.0006);//30,140,170,120,-0.0006,0.0006);
TH2D *h_res_mu_inTR=new TH2D("h_res_mu_inTR", "(thmu_in_rec-thmu_in_true) VS energy Emu(GeV) POST-VRTX",10,150,160,22,-0.0006,0.0006);

TH1D *h_res_muTR1x=new TH1D("h_res_muTR1x", "(thmu_rec-thmu_true) proj. X  155<Emu<157(GeV) POST-VRTX",120,-0.0006,0.0006);
TH1D *h_res_muTR2x=new TH1D("h_res_muTR2x", "(thmu_rec-thmu_true) proj. X 157<Emu<160(GeV) POST-VRTX",120,-0.0006,0.0006);
TH1D *h_res_muTR3x=new TH1D("h_res_muTR3x", "(thmu_rec-thmu_true) proj. X 158<Emu<160(GeV) POST-VRTX",120,-0.0006,0.0006);

TH1D *h_res_muTR1y=new TH1D("h_res_muTR1y", "(thmu_rec-thmu_true) proj. Y 155<Emu<157(GeV) POST-VRTX",120,-0.0006,0.0006);
TH1D *h_res_muTR2y=new TH1D("h_res_muTR2y", "(thmu_rec-thmu_true) proj. Y 157<Emu<160(GeV) POST-VRTX",120,-0.0006,0.0006);
TH1D *h_res_muTR3y=new TH1D("h_res_muTR3y", "(thmu_rec-thmu_true) proj. Y 158<Emu<160(GeV) POST-VRTX",120,-0.0006,0.0006);

TH1D *h_res_muTR1=new TH1D("h_res_muTR1", "(thmu_rec-thmu_true) VS energy 155<Emu<157(GeV) POST-VRTX",180,-0.0006,0.0006);
TH1D *h_res_muTR2=new TH1D("h_res_muTR2", "(thmu_rec-thmu_true) VS energy 157<Emu<160(GeV) POST-VRTX",180,-0.0006,0.0006);
TH1D *h_res_muTR3=new TH1D("h_res_muTR3", "(thmu_rec-thmu_true) VS energy 158<Emu<160(GeV) POST-VRTX",180,-0.0006,0.0006);
TH1D *h_res_mu_inTR1=new TH1D("h_res_mu_inTR1", "(thmu_in_rec-thmu_in_true)  POST-VRTX",80,-0.0002,0.0002);
...
double thmu_rec_vrtx_x=0; double thmu_rec_vrtx_y=0;
...
if(j==0 and code_mu==mu_out.linkedTrackID() and code_e==e_out.linkedTrackID() and tracks.size()==3){
        yes_v++;
...
thmu_rec_vrtx_x=mu_out.xSlope();
thmu_rec_vrtx_y=mu_out.ySlope();
...
if(yes_mu==1 and yes_e==1 and yes_v>0){reco+=MesmerEvent->wgt_full;

double resTR = the_rec_vrtx-the_gen;
double res_muTR = thmu_rec_vrtx-thmu_gen;
double res_muinTR = th_in_rec_vrtx-th_in_gen;
double resx=thmu_rec_vrtx_x-thmu_gen_x;
double resy=thmu_rec_vrtx_y-thmu_gen_y;
...
if(abs(resx)<0.1e-03 and abs(resy)<0.1e-03) {   theta1x->Fill(thmu_rec_vrtx_x,MesmerEvent->wgt_full);theta1gx->Fill(thmu_gen_x,MesmerEvent->wgt_full);
						theta1y->Fill(thmu_rec_vrtx_y,MesmerEvent->wgt_full);theta1gy->Fill(thmu_gen_y,MesmerEvent->wgt_full);}


if(abs(resx)>0.1e-03){tailx+=MesmerEvent->wgt_full;
				   theta2x->Fill(thmu_rec_vrtx_x,MesmerEvent->wgt_full);theta2gx->Fill(thmu_gen_x,MesmerEvent->wgt_full);
				   theta2y_if->Fill(thmu_rec_vrtx_y,MesmerEvent->wgt_full);theta2gy_if->Fill(thmu_gen_y,MesmerEvent->wgt_full);}
if(abs(resy)>0.1e-03){taily+=MesmerEvent->wgt_full;
                                   theta2y->Fill(thmu_rec_vrtx_y,MesmerEvent->wgt_full);theta2gy->Fill(thmu_gen_y,MesmerEvent->wgt_full);
				   theta2x_if->Fill(thmu_rec_vrtx_x,MesmerEvent->wgt_full);theta2gx_if->Fill(thmu_gen_x,MesmerEvent->wgt_full);}
...
cout << "Su " << signal << " eventi di segnale, eventi di con proiezione x nella coda " << (peakx/reco)*100 << "%"<< endl;
cout << "Su " << signal << " eventi di segnale, eventi di con proiezione y nella coda " << (peaky/reco)*100 << "%"<< endl;
