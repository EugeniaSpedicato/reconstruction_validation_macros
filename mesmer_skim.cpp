...
///mnt/raid10/DATA/espedica/fairmu/theta_0-32mrad_1M_1hit_NOoutchi2_NOTILT.root");//32mrad_1M_1hit_NOoutchi2.root");
...
string title="Number of stubs per module "+to_string(m);
...
stub_per_mod_after.at(m)=new TH1D(name.c_str(),title.c_str(),20,0,20);
  ...
  TH2D *h2_nStubs = new TH2D("h2_nStubs","N stubs on S1 vs S0, with 1PUmu",nb,0,nmax,nb,0,nmax);
  TH1D *h_nStubs_0 = new TH1D("h_nStubs_0","Tot N stubs First Station - S0, with 1PUmu",nb,0,nmax);
  TH1D *h_nStubs_1 = new TH1D("h_nStubs_1","Tot N stubs Second Station - S1, with 1PUmu",nb,0,nmax);

  TH1D *h_nTotStubs = new TH1D("h_nTotStubs","Total N of Stubs with 1PUmu",nb,0,nmax);
...
TH1D *theta_mu_gen=new TH1D("theta_mu_gen", "Muon scattering generated angles from MESMER",25,0.,0.005);
TH1D *theta_e_gen_after=new TH1D("theta_e_gen_after", "Electron scattering generated angles from MESMER after skim",64,0.,0.032);
TH1D *theta_mu_gen_after=new TH1D("theta_mu_gen_after", "Muon scattering generated angles from MESMER after skim",25,0.,0.005);
...
if(mu_gen==1 and e_out!=-99 and mu_out!=-99 and hit_modXmuin==4 and hit_modYmuin==4 and stereo_muin>1 and hit_modXe==4 and hit_modYe==4 and stereo_e>1 and hit_modXmu==4 and hit_modYmu==4 and stereo_mu>1 and the_gen>0.0025){

N_input_events++;

N_input_stubs += TrackerStubs->GetEntries();

h_nTotStubs->Fill(TrackerStubs->GetEntries(),MesmerEvent->wgt_full);
...
//if(nTotStubs>=15){cout << "event skim >=15 " << i << endl;}
//if(nTotStubs<15){cout << "event skim <15 " << i << endl;}
...
/*else{
  if(nTotStubs>=15){cout << "event NO skim >=15 " << i << endl;}
  if(nTotStubs<15){cout << "event NO skim <15 " << i << endl;}
}*/
...
h3->Draw("E");
gStyle->SetOptStat(0);
a1.SaveAs("eff_skim.pdf");
h2->SaveAs("eff_skim_e.root");
...
*/
  ...
  cout<<"------------------------" << endl;
  cout<<"Number of filtered event: " << n_filtered << " over the number of total reconstructible events generated: " << n_tot << " -> " << n_filtered/n_tot*100 << "%" << endl;
