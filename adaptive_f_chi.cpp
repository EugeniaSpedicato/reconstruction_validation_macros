...
void RealDataAnalyzer(){
 	...
 	TFile *inputfile = new TFile("TRMesmer_2_2stations.root");//narrowBP_20GeV_1shared.root");//trPoints,20GeV.root");//exampleProductionJob.root");narrowBP.root
...
TH2D *h_res_mu=new TH2D("h_res_mu", "(thmu_rec-thmu_true) VS energy Emu(GeV) POST-VRTX",30,145,175,22,-0.0006,0.0006);//30,140,170,22,-0.0006,0.0006);
TH2D *h_res_mu_in=new TH2D("h_res_mu_in", "(thmu_in_rec-thmu_in_true) VS energy Emu(GeV) POST-VRTX",30,145,175,22,-0.0006,0.0006);
...
TH2D *h_res_muTR=new TH2D("h_res_muTR", "(thmu_rec-thmu_true) VS energy Emu(GeV) PRE-VRTX",30,145,175,22,-0.0006,0.0006);//30,140,170,22,-0.0006,0.0006);
TH2D *h_res_mu_inTR=new TH2D("h_res_mu_inTR", "(thmu_in_rec-thmu_in_true) VS energy Emu(GeV) PRE-VRTX",30,145,175,22,-0.0006,0.0006);
...
TH1D *h_ae=new TH1D("h_ae","angle of electron track from vertexing when linked to a particle different from e-", 500,0.,0.05);
TH1D *h_amu=new TH1D("h_amu","angle of muon track from vertexing when linked to a particle different from mu-", 50,0.,0.005);


double the_gen=0; double thmu_gen=0; double thmu=0; double the=0;
double th_in_gen=0; 
double the_rec=0;double thmu_rec=0;
double th_in_rec=0;
double the_recTR=0;double thmu_recTR=0;
double th_in_recTR=0;
...
double Emu_in;
	...
	th_in_gen=pmuin.Theta();
	 ...
	 double energy=MCTr->energy(); Emu_in=energy;
	 double mx=MCTr->ax();
	 double my=MCTr->ay();
	 double qx=MCTr->bx();
         double qy=MCTr->by();
	 double x1= qx+mx*(118.0218);
         ...
         muin.SetXYZ(mx,my,sqrt(mx*mx+my*my));

         cout << " X and Y divergence mu_in (rad): " << mx << ", " << my << endl;
	...
	const MUonETrack *SigTracks = static_cast<const MUonETrack*>(SignalTracks->At(n));
	 ...
	 { 
	   ...
	   int stereo_mu=0; int stereo_e=0;
           ...
           if(SigTracks->pdgCode()==11 and last_modXe==2 and last_modYe==2 and stereo_e>1){ // and abs(SigTracks->startX())<2 and abs(SigTracks->startY())<2 ){
		pe.SetXYZ(SigTracks->px(),SigTracks->py(),SigTracks->pz()); the_gen=pe.Theta();
		Ee=SigTracks->energy();
		double mx1=SigTracks->ax();
		double my1=SigTracks->ay();
		the=sqrt(mx1*mx1+my1*my1);
		pe_dir=pe.Unit();
		TVector3 e(mx1,my1,1.0);
                the_sdr=muin.Angle(e);
		//the_sdr=acos(pmuin_dir.Dot(pe_dir));
		yes_e=1;}//if(the_sdr<0.035)yes_e=1;}
 	   if(SigTracks->pdgCode()==13 and last_modXmu==2 and last_modYmu==2 and stereo_mu>1){ // and abs(SigTracks->startX())<2 and abs(SigTracks->startY())<2 ){ 
		Emu=SigTracks->energy();
		pmu.SetXYZ(SigTracks->px(),SigTracks->py(),SigTracks->pz());
		thmu_gen=pmu.Theta();
		double mx1=SigTracks->ax();
		double my1=SigTracks->ay();
		thmu=sqrt(mx1*mx1+my1*my1);
                pmu_dir=pmu.Unit();
...
in.SetXYZ(th_inx,th_iny,sqrt(th_inx*th_inx+th_iny*th_iny)); 
th_in_recTR=sqrt(th_inx*th_inx+th_iny*th_iny); //cout << in.Theta() << " vs in gen " << th_in_gen << endl;

//th_in_recTR=sqrt(tracks.at(j).xSlope()*tracks.at(j).xSlope()+tracks.at(j).ySlope()*tracks.at(j).ySlope());
}
}
...
cout << "vrtx.at(j).z() " << vrtx.at(j).z() << " tr size " << tracks.size() << " event " << i << endl;

   if(vrtx.at(j).stationIndex()==0) //and tracks.at(0).processIDofLinkedTrack()==45 and tracks.at(0).linkedTrackID()!=tracks.at(1).linkedTrackID()){
	{
	for(int v=0; v<vrtx.at(j).tracks().size(); v++)
	 { cout << "id " << tr_ad.at(v).linkedTrackID() << endl;
	 if(tr_ad.at(v).linkedTrackID()==0 and tr_ad.at(v).sector()==0) {mu_in = tr_ad.at(v); cout << "ciaooo " << mu_in.xSlope() <<endl;}
	 }
...
TVector3 in1(mu_in.xSlope(),mu_in.ySlope(),sqrt(mu_in.xSlope()*mu_in.xSlope()+mu_in.ySlope()*mu_in.ySlope()));
th_in_rec=sqrt(mu_in.xSlope()*mu_in.xSlope()+mu_in.ySlope()*mu_in.ySlope()); //cout << in.Theta() << " vs in gen " << th_in_gen << endl;
...
if(code_e!=e_out.linkedTrackID()) {e+=MesmerEvent->wgt_full;
					...
					h_ae->Fill(the_sdr,MesmerEvent->wgt_full);}
if(code_mu!=mu_out.linkedTrackID()) {mu+=MesmerEvent->wgt_full;
					...
					h_amu->Fill(thmu_sdr,MesmerEvent->wgt_full);}

if(tracks.size()==3){ //and e_out.chi2perDegreeOfFreedom()<50 and mu_out.chi2perDegreeOfFreedom()<50){
cout << mu_in.xSlope() <<endl;
TVector3 mu1(mu_out.xSlope(),mu_out.ySlope(),1.0);
TVector3 e1(e_out.xSlope(),e_out.ySlope(),1.0);


	the_rec = in.Angle(e1);//vrtx.at(j).electronTheta();
        thmu_rec = in.Angle(mu1);//vrtx.at(j).muonTheta();

cout << "th_mu_rec " << thmu_rec << " vs thmu_sdr " << thmu_sdr << endl;


/*	the_rec=sqrt(e_out.xSlope()*e_out.xSlope()+e_out.ySlope()*e_out.ySlope());
	thmu_rec=sqrt(mu_out.xSlope()*mu_out.xSlope()+mu_out.ySlope()*mu_out.ySlope());
        th_in_rec=sqrt(mu_in.xSlope()*mu_in.xSlope()+mu_in.ySlope()*mu_in.ySlope());
*/
...
if(code_e==tracks.at(j).linkedTrackID()) {h_chi_eTOT->Fill(tracks.at(j).chi2(),MesmerEvent->wgt_full);
				//the_recTR=sqrt(tracks.at(j).xSlope()*tracks.at(j).xSlope()+tracks.at(j).ySlope()*tracks.at(j).ySlope());
       //the_recTR =sqrt((th_inx-tracks.at(j).xSlope())*(th_inx-tracks.at(j).xSlope())+(th_iny-tracks.at(j).ySlope())*(th_iny-tracks.at(j).ySlope()));

	TVector3 e_outv(tracks.at(j).xSlope(),tracks.at(j).ySlope(),1.0);
        the_recTR=in.Angle(e_outv);

		}
if(code_mu==tracks.at(j).linkedTrackID()) {h_chi_muTOT->Fill(tracks.at(j).chi2(),MesmerEvent->wgt_full);
		   			  //thmu_recTR=sqrt(tracks.at(j).xSlope()*tracks.at(j).xSlope()+tracks.at(j).ySlope()*tracks.at(j).ySlope());
	TVector3 mu_outv(tracks.at(j).xSlope(),tracks.at(j).ySlope(),1.0);
       //thmu_recTR =sqrt((th_inx-tracks.at(j).xSlope())*(th_inx-tracks.at(j).xSlope())+(th_iny-tracks.at(j).ySlope())*(th_iny-tracks.at(j).ySlope()));
	//mu_outv=mu_outv.Unit();
	thmu_recTR=in.Angle(mu_outv);//acos(in.Dot(mu_outv));//in.Angle(mu_outv); 
	//thmu_recTR=mu_out.Theta();
		}
}
...
if(yes_v>=1){
double res = the_rec-the_sdr;
double res_mu = thmu_rec-thmu_sdr;
double res_muin = th_in_rec-th_in_gen;

double resTR = the_recTR-the_sdr;
double res_muTR = thmu_recTR-thmu_sdr;
double res_muinTR = th_in_recTR-th_in_gen;
