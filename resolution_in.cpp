...
if(last_modXmu==2 and last_modYmu==2 and stereo_mu>1){
		yes_mu=1;
			}
...
for(int j=0; j<tracks.size();j++)
{
 if(tracks.at(j).processIDofLinkedTrack()==0 and tracks.size()==2 and tracks.at(j).sector()==1 and tracks.at(j).percentageOfHitsSharedWithLinkedTrack()>=100)
	{yes2++;


		 if(0==tracks.at(j).linkedTrackID()) { yes_mu=1;
...
double res_muTR = thmu_rec_tracks-th_in_gen;
double res_muinTR = th_in_rec_tracks-th_in_gen;
double resx=thmu_rec_x-thmu_in_gen_x;
double resy=thmu_rec_y-thmu_in_gen_y;
...
if(th_in_gen<=0.0003) {h_res_mu_in_TR1->Fill(res_muinTR);h_res_mu_in_TR1x->Fill(resx_in); h_res_mu_in_TR1y->Fill(resy_in);}
if(th_in_gen<=0.0007 and th_in_gen>0.0003) {h_res_mu_in_TR2->Fill(res_muinTR);h_res_mu_in_TR2x->Fill(resx_in); h_res_mu_in_TR2y->Fill(resy_in);}
if(th_in_gen>0.0007) {h_res_mu_in_TR3->Fill(res_muinTR);h_res_mu_in_TR3x->Fill(resx_in); h_res_mu_in_TR3y->Fill(resy_in);}


if(th_in_gen<=0.0003) {h_res_muTR1->Fill(res_muTR);h_res_muTR1x->Fill(resx); h_res_muTR1y->Fill(resy);}
if(th_in_gen<=0.0007 and thmu_gen>0.0003) {h_res_muTR2->Fill(res_muTR);h_res_muTR2x->Fill(resx); h_res_muTR2y->Fill(resy);}
if(th_in_gen>0.0007) {h_res_muTR3->Fill(res_muTR);h_res_muTR3x->Fill(resx); h_res_muTR3y->Fill(resy);}
h_res_muTR->Fill(Emu,res_muTR);
h_res_mu_inTR->Fill(Emu_in,res_muinTR);

h_res_mu_inTR_all->Fill(res_muinTR);

if(res_muTR>0.1e-03){theta1->Fill(thmu_rec_tracks);theta1g->Fill(th_in_gen);}
if(res_muTR<0.1e-03 and res_muTR>-0.1e-03){theta2->Fill(thmu_rec_tracks);theta2g->Fill(th_in_gen);}


if(abs(resx)<0.1e-03 and abs(resy)<0.1e-03) {theta1x->Fill(thmu_rec_x);theta1gx->Fill(thmu_in_gen_x);
						theta1y->Fill(thmu_rec_y);theta1gy->Fill(thmu_in_gen_y);}

if(abs(resx)>0.1e-03){theta2x->Fill(thmu_rec_x);theta2gx->Fill(thmu_in_gen_x);
				   theta2y_if->Fill(thmu_rec_y);theta2gy_if->Fill(thmu_in_gen_y);}
if(abs(resy)>0.1e-03){theta2y->Fill(thmu_rec_y);theta2gy->Fill(thmu_in_gen_y);
				   theta2x_if->Fill(thmu_rec_x);theta2gx_if->Fill(thmu_in_gen_x);}
