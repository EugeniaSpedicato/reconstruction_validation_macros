...
if(m_reconstructionConfiguration.verbose())
std::cout << "Reference track over " <<  reconstructedTracksPerSector[sector].size() << " tracks"<< std::endl;
...
auto sh=std::find_if(it + 1, end, [&track_stereo_hits](MUonERecoTrack3D& track){
...
std::cout <<" sh!=reconstructedTracksPerSector[sector].end()? " << std::endl;

auto& sh_tr=*sh;

if(sh==reconstructedTracksPerSector[sector].end()){
++it;
		...
		if(m_reconstructionConfiguration.verbose()) std::cout << "stereo hits shared track " << sh_tr.stereoHits().size() << std::endl;
			...
			for(Int_t h=0; h<sh_track_stereo_hits.size(); h++){
			 if(std::find(track_stereo_hits.begin(),track_stereo_hits.end(), sh_track_stereo_hits.at(h))!=track_stereo_hits.end()){removed_hit.push_back(track_stereo_hits.at(h));
																	if(m_reconstructionConfiguration.verbose()) std::cout << "index removed hit " << track_stereo_hits.at(h).index() << std::endl;}
			}
...
if(m_reconstructionConfiguration.verbose())
{std::cout << r << "th removed hit at module " << removed_hit.at(r).module().index() << std::endl;
...
MUonERecoHit rhit=removed_hit.at(r);
                        ...
                        hits_per_module_removed[removed_hit.at(r).module().index()].erase(std::remove_if(hits_per_module_removed[removed_hit.at(r).module().index()].begin(), hits_per_module_removed[removed_hit.at(r).module().index()].end(), [&rhit](MUonERecoHit const& hit){
...
if(m_reconstructionConfiguration.verbose())
std::cout << "how many stereo!? " << nstereo << std::endl;
		...
		if(m_reconstructionConfiguration.verbose()) std::cout << "for loop " << stubmodule_index << std::endl;
				...
				{
				if(m_reconstructionConfiguration.verbose()) std::cout << "addfit info yes " << std::endl;
...
auto sh=std::find_if(it + 1, reconstructedTracksPerSector[sector].end(), [&track_stereo_hits](MUonERecoTrack3D& track){
...
std::cout <<" sh!=reconstructedTracksPerSector[sector].end() " << std::endl;
				...
				while(sh!=reconstructedTracksPerSector[sector].end()){
auto& sh_tr=*sh;
...
std::cout << "how many stereo in module "<<removed_hit.at(r).module().index()<< "!? " << nstereo << std::endl;
               ...
               if(m_reconstructionConfiguration.verbose()) std::cout << "i am trivial" << std::endl;
                                //reconstructedTracksPerSector[sector].emplace_back(std::move(sh_tr));
					reconstructedTracksPerSector[sector].erase(std::remove_if(it + 1, reconstructedTracksPerSector[sector].end(), [&](MUonERecoTrack3D& track) {auto count = track.numberOfXYHitsSharedWith(*it, m_reconstructionConfiguration.restrictHitSharingToFirstNModulesInSector()); return (count.first > m_reconstructionConfiguration.maxNumberOfSharedHits() || count.second > 0);}), reconstructedTracksPerSector[sector].end());
...
<< std::endl;}
	 ...
	 sh=std::find_if(it + 1, reconstructedTracksPerSector[sector].end(), [&track_stereo_hits](MUonERecoTrack3D& track){
                if(std::find_if(track_stereo_hits.begin(),track_stereo_hits.end(), [&](MUonERecoHit const& hit){
	...
	if(m_reconstructionConfiguration.verbose()) std::cout << "not enough stereo hits " << std::endl;
				reconstructedTracksPerSector[sector].erase(std::remove_if(it + 1, reconstructedTracksPerSector[sector].end(), [&](MUonERecoTrack3D& track) {auto count = track.numberOfHitsSharedWith(*it, m_reconstructionConfiguration.restrictHitSharingToFirstNModulesInSector()); return (count.first > m_reconstructionConfiguration.maxNumberOfSharedHits() || count.second > 0);}), reconstructedTracksPerSector[sector].end());
        if(m_reconstructionConfiguration.verbose()) std::cout << "removed?"<<std::endl;
                                ++it;
                                end = reconstructedTracksPerSector[sector].end();

         sh=std::find_if(it + 1, end, [&track_stereo_hits](MUonERecoTrack3D& track){
                if(std::find_if(track_stereo_hits.begin(),track_stereo_hits.end(), [&](MUonERecoHit const& hit){
         ...
         if(m_reconstructionConfiguration.verbose()) std::cout << "end while(sh!=end)"<<std::endl;
					}//end if(sh!=end)
				}//end while it!=end
