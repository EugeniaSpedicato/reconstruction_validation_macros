...
void vrtx_efficiency(int nhits){
...
TH1D *theta_e_clone_quality=new TH1D("theta_e_clone_quality", "Reco elastic event: Electron scattering reco angles from MESMER quality check",NBINS,edges_el);
TH1D *theta_mu_clone_quality=new TH1D("theta_mu_clone_quality", "Reco elastic event: Muon scattering reco angles from MESMER quality check",NBINS_mu,edges_mu);
...
TH1D *h_opening_gen=new TH1D("h_opening_gen", "Opening angle generated from MESMER",33,0.,0.033);

TH1D *h_opening_clone=new TH1D("h_opening_clone", "Opening angle reco events from MESMER with clones",33,0.,0.033);

TH1D *h_opening_clone_quality=new TH1D("h_opening_clone_quality", "Opening angle reco events from MESMER quality check",33,0.,0.033);

TH1D *h_opening_clone_vrtx=new TH1D("h_opening_clone_vrtx", "True opening angle reco events from MESMER when vrtx is created with good quality",33,0.,0.033);

TH1D *h_opening_wrong_vrtx=new TH1D("h_opening_wrong_vrtx", "True opening angle reco events from MESMER when vrtx is created with wrong association",33,0.,0.033);
...
// Define best vertex and its chi2 and look at the all-vrtx vector
...
if(stubs_muin>=5 and chi2_muin<2){
...
//pre-vrtx
 ...
 if( ((e>=1 and mu>1) or (e>1 and mu>=1)) and chi!=0)clones+=MesmerEvent->wgt_LO*wnorm;

 if(e>=1 and mu>=1 and chi!=0) h_opening_clone->Fill(opening_angle,MesmerEvent->wgt_LO*wnorm);


 //look at vrtx PID and MC truthg
 if( (vrtx.outgoingMuon().linkedTrackID()!=code_e and vrtx.outgoingMuon().linkedTrackID()!=code_mu) or (vrtx.outgoingElectron().linkedTrackID()!=code_mu and vrtx.outgoingElectron().linkedTrackID()!=code_e) ){
        n_wrong+=MesmerEvent->wgt_LO*wnorm;
		for(int n = 0; n < MCTrack->GetEntries(); n++) {
                 const MUonETrack *MCTr = static_cast<const MUonETrack*>(MCTrack->At(n));
                if(n==vrtx.outgoingMuon().linkedTrackID()) h_process_mu->Fill(MCTr->interactionID(),MesmerEvent->wgt_LO*wnorm);
                if(n==vrtx.outgoingElectron().linkedTrackID()) h_process_el->Fill(MCTr->interactionID(),MesmerEvent->wgt_LO*wnorm);
                }
	}
 else if(vrtx.outgoingMuon().linkedTrackID()==code_mu and vrtx.outgoingElectron().linkedTrackID()==code_e)n_correct+=MesmerEvent->wgt_LO*wnorm;
 else if(vrtx.outgoingMuon().linkedTrackID()==code_e and vrtx.outgoingElectron().linkedTrackID()==code_mu)n_swapped+=MesmerEvent->wgt_LO*wnorm;

// quality checks on vrtx tracks for elastic event reco efficiency
...
if(chi!=0 and e==1 and mu==1)no_clones_q+=MesmerEvent->wgt_LO*wnorm;
if(chi!=0 and ((e>=1 and mu>1) or (e>1 and mu>=1)))clones_q+=MesmerEvent->wgt_LO*wnorm;
...
if(chi!=0 and e>=1 and mu>=1){
			...
			theta_e_clone_quality->Fill(the_gen,MesmerEvent->wgt_LO*wnorm);
         		h_opening_clone_quality->Fill(opening_angle,MesmerEvent->wgt_LO*wnorm);
				}
			}//if quality vrtx
...
// If the particles associeted to the vrtx tracks are 1 mu + 1 e (also with swapped identity), then the vrtx did it well, otherwise count it as wrong
 if(e>=1 and mu>=1 and chi!=0){
  ...
  else{h_opening_wrong_vrtx->Fill(opening_angle,MesmerEvent->wgt_LO*wnorm);}
}
...
cout << "fractions elastic events without quality " << clones/tot*100 << "%" << endl;

cout << "fractions elastic events with quality without clones " << no_clones_q/tot*100 << "%" << endl;
cout << "fractions elastic events with quality " << clones_q/tot*100 << "%" << endl;
...
n5.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/h_process_%dhit.pdf",static_cast<char>(nhits)));


TH1D * h0gen = (TH1D*) theta_e_gen->Clone();
TH1D * h1gen = (TH1D*) theta_mu_gen->Clone();
TH1D * hgen = (TH1D*) h_opening_gen->Clone();
...
TH1D * h = (TH1D*) h_opening_clone_quality->Clone();
h->Divide(h,hgen,1,1,"B");
h->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/quality_op_gen_%dhit.root",static_cast<char>(nhits)));
...
h0->SetTitle("Reco eff. elastic event as a funtion of e- angle");
h0->SetMinimum(0.6);
h0->Draw("E");
h0->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/quality_el_gen_%dhit.root",static_cast<char>(nhits)));
gStyle->SetOptStat(0);
n7.cd(2);
TGaxis::SetMaxDigits(3);
h1->SetTitle("Reco eff. elastic event as a funtion of #mu angle");
h1->SetMinimum(0.6);
h1->Draw("E");
h1->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/quality_mu_gen_%dhit.root",static_cast<char>(nhits)));
gStyle->SetOptStat(0);
n7.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/eff_vrtx_quality_%dhit.pdf",static_cast<char>(nhits)));
...
a.cd(2);
h2vrtx_w->Draw();
a.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/vrtx_LO_%dhit_quality.pdf",static_cast<char>(nhits)));

h2vrtx->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/vrtx_correct_LO_%dhit_true_clone_quality.root",static_cast<char>(nhits)));
h2vrtx_w->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/vrtx_wrong_LO_%dhit_true_clone_quality.root",static_cast<char>(nhits)));
