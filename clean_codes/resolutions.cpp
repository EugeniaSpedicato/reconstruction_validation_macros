...
void resolutions(int nhits, int version){

TChain * cbmsim = new TChain("cbmsim");
TChain * cbmsim_g = new TChain("cbmsim");
TString gen_filename;
 TTree *t2;
if(nhits==0 and version==0){
...
else if((nhits==1 or nhits==2)  and version==0){
...
//cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/efficiency_NLO/single_sample_%dhitFirstModules_NOoutchi2_MCcorrections_v3_0.root",static_cast<char>(nhits)));
gen_filename ="/mnt/raid10/DATA/espedica/fairmu/efficiency_NLO/old_1hitFirstModules.root";
cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/efficiency_NLO/theta_0-32_mrad_%dhitFirstModules_NOoutchi2_reassign_MCcorrections.root",static_cast<char>(nhits)));
 TFile *f2 = new TFile(gen_filename);
 t2 = (TTree*)f2->Get("cbmsim");
 cbmsim->AddFriend(t2);
}
else if(version==2){
	  cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/reco/old_WiP/WiP_v0140_commit_0996edd4_0_5mrad_%dhitFirstModules.root",static_cast<char>(nhits)));
          cbmsim_g->Add("/mnt/raid10/DATA/espedica/fairmu/gen_digi/WiP_v0140_commit_258faf6b_0_5mrad.root");

          cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/reco/old_WiP/WiP_v0140_commit_0996edd4_5_10mrad_%dhitFirstModules.root",static_cast<char>(nhits)));
          cbmsim_g->Add("/mnt/raid10/DATA/espedica/fairmu/gen_digi/WiP_v0140_commit_258faf6b_5_10mrad.root"); 

          cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/reco/old_WiP/WiP_v0140_commit_0996edd4_10_15mrad_%dhitFirstModules.root",static_cast<char>(nhits)));
          cbmsim_g->Add("/mnt/raid10/DATA/espedica/fairmu/gen_digi/WiP_v0140_commit_258faf6b_10_15mrad.root"); 

          cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/reco/old_WiP/WiP_v0140_commit_0996edd4_15_20mrad_%dhitFirstModules.root",static_cast<char>(nhits)));
          cbmsim_g->Add("/mnt/raid10/DATA/espedica/fairmu/gen_digi/WiP_v0140_commit_258faf6b_15_20mrad.root"); 

          cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/reco/old_WiP/WiP_v0140_commit_0996edd4_20_25mrad_%dhitFirstModules.root",static_cast<char>(nhits)));
          cbmsim_g->Add("/mnt/raid10/DATA/espedica/fairmu/gen_digi/WiP_v0140_commit_258faf6b_20_25mrad.root"); 

          cbmsim->Add(Form("/mnt/raid10/DATA/espedica/fairmu/reco/old_WiP/WiP_v0140_commit_0996edd4_25_32mrad_%dhitFirstModules.root",static_cast<char>(nhits)));
          cbmsim_g->Add("/mnt/raid10/DATA/espedica/fairmu/gen_digi/WiP_v0140_commit_258faf6b_25_32mrad.root"); 



cout << "cbmsim " << cbmsim->GetEntries() << endl;
cout << "cbmsim_g "<< cbmsim_g->GetEntries() << endl;
...
double r_wnorm[6];
	if(version==0){r_wnorm[0]=20.786765103274643;r_wnorm[1]=33.313091221576336;r_wnorm[2]=42.396733790329876;r_wnorm[3]=50.584815206143652;r_wnorm[4]=61.828110400735824;r_wnorm[5]=106.88513370134392;}
	else if(version==1){r_wnorm[0]=1.;r_wnorm[1]=1.;r_wnorm[2]=1.;r_wnorm[3]=1.;r_wnorm[4]=1.;r_wnorm[5]=1.;}
	else if(version==2){r_wnorm[0]=18.558632798881270;r_wnorm[1]=31.156709183434657;r_wnorm[2]=41.306609022934722;r_wnorm[3]=50.678758808390178;r_wnorm[4]=61.782157006784871;r_wnorm[5]=106.98964295859898;}
...
// wnorm is a number needed for normalization when we use different mesmer sample together (like in this case, 6 sample in differen kinematic region of the electron). We also register an index for plots dipendently on the mu/el angle for the version event

if(the_gen>=0. and the_gen<0.001){index=0; wnorm=r_wnorm[0];}
else if(the_gen>=0.001 and the_gen<0.002){index=1; wnorm=r_wnorm[0];}
else if(the_gen>=0.002 and the_gen<0.003){index=2; wnorm=r_wnorm[0];}
else if(the_gen>=0.003 and the_gen<0.004){index=3; wnorm=r_wnorm[0];}
else if(the_gen>=0.004 and the_gen<0.005){index=4; wnorm=r_wnorm[0];}
else if(the_gen>=0.005 and the_gen<0.006){index=5; wnorm=r_wnorm[1];}
else if(the_gen>=0.006 and the_gen<0.007){index=6; wnorm=r_wnorm[1];}
else if(the_gen>=0.007 and the_gen<0.008){index=7; wnorm=r_wnorm[1];}
else if(the_gen>=0.008 and the_gen<0.009){index=8; wnorm=r_wnorm[1];}
else if(the_gen>=0.009 and the_gen<0.010){index=9; wnorm=r_wnorm[1];}
else if(the_gen>=0.010 and the_gen<0.015){index=10; wnorm=r_wnorm[2];}
else if(the_gen>=0.015 and the_gen<0.020){index=11; wnorm=r_wnorm[3];}
else if(the_gen>=0.020 and the_gen<0.025){index=12; wnorm=r_wnorm[4];}
else if(the_gen>=0.025 and the_gen<=0.033){index=13; wnorm=r_wnorm[5];}
else continue;


if(thmu_gen>=0. and thmu_gen<0.0001)index_mu=0;
else if(thmu_gen>=0.0001 and thmu_gen<0.0002)index_mu=1;
else if(thmu_gen>=0.0002 and thmu_gen<0.0003)index_mu=2;
else if(thmu_gen>=0.0003 and thmu_gen<0.0004)index_mu=3;
else if(thmu_gen>=0.0004 and thmu_gen<0.0005)index_mu=4;
else if(thmu_gen>=0.0005 and thmu_gen<0.0006)index_mu=5;
else if(thmu_gen>=0.0006 and thmu_gen<0.0007)index_mu=6;
else if(thmu_gen>=0.0007 and thmu_gen<0.0008)index_mu=7;
else if(thmu_gen>=0.0008 and thmu_gen<0.0009)index_mu=8;
else if(thmu_gen>=0.0009 and thmu_gen<0.001)index_mu=9;
else if(thmu_gen>=0.001 and thmu_gen<0.002)index_mu=10;
else if(thmu_gen>=0.002 and thmu_gen<0.003)index_mu=11;
else if(thmu_gen>=0.003 and thmu_gen<0.004)index_mu=12;
else if(thmu_gen>=0.004 and thmu_gen<=0.005)index_mu=13;
else continue;
...
h_res_el_pre.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_prevrtx_el_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m), static_cast<char>(nhits)));
h_res_vrtx_el.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_vrtx_el_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m), static_cast<char>(nhits)));
//gPad->SetLogy();
}
n0.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/h_res_pre_el_%dhit_LO.pdf",static_cast<char>(nhits)));
...
h_res_el_pre_clones.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_prevrtx_clones_el_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m), static_cast<char>(nhits)));
h_res_vrtx_el_clones.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_vrtx_clones_el_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m), static_cast<char>(nhits)));
}
n1.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/h_res_vrtx_el_clones_%dhitFirstModules_LO.pdf",static_cast<char>(nhits)));
...
h_res_mu_pre.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_prevrtx_AngleMu_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m), static_cast<char>(nhits)));
h_res_vrtx_mu.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_vrtx_AngleMu_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m), static_cast<char>(nhits)));
//gPad->SetLogy();
}
n2.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/h_res_pre_mu_%dhit_LO.pdf",static_cast<char>(nhits)));
...
h_res_mu_pre_clones.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_prevrtx_clones_AngleMu_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m),static_cast<char>(nhits)));
h_res_vrtx_mu_clones.at(m)->SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/res_vrtx_clones_AngleMu_%d_%dhit_LO_newTuple_MS.root",static_cast<char>(m),static_cast<char>(nhits)));
}
n3.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/h_res_vrtx_mu_clones_%dhit_LO.pdf",static_cast<char>(nhits)));

TCanvas in("in","in",700,700);
h_res_vrtx_muin->Draw();
in.SaveAs(Form("/home/espedica/macros_fairmu/clean_codes/validation/h_res_pmuin_%dhit_LO.pdf",static_cast<char>(nhits)));
